...
registerHooks({
  resolve(specifier, context, next) {
    return next(
      specifier.startsWith('.') && !path.extname(specifier) ? `${specifier}.ts` : specifier,
      context,
    );
  },
});
...
const references = {
  crowns: findSurfaceIntersections(reference, {}),
  roots: findSurfaceIntersections(rootModel(reference), {}),
};
...
function intersections(model, transforms, component) {
  ...
  const changed = new Set(
    Object.entries(transforms)
      .filter(([, value]) =>
        [...value.translation, ...value.rotation].some(n => Math.abs(n) > 1e-12),
      )
      .map(([id]) => id),
  );
  const cacheKey = JSON.stringify([component, [...present], transforms]);
  if (cache.has(cacheKey)) return cache.get(cacheKey);
  const pairs = references[component].filter(
    pair =>
      present.has(pair.a) && present.has(pair.b) && !changed.has(pair.a) && !changed.has(pair.b),
  );
  for (let i = 0; i < teeth.length; i++)
    for (let j = i + 1; j < teeth.length; j++) {
      if (!changed.has(teeth[i].id) && !changed.has(teeth[j].id)) continue;
      pairs.push(
        ...findSurfaceIntersections({ ...model, teeth: [teeth[i], teeth[j]] }, transforms),
      );
    }
  pairs.sort((a, b) => pairKey(a).localeCompare(pairKey(b)));
...
try {
  ...
  for (const definition of TEACHING_CASES) {
    ...
    const baseline = {
      crownPairs: intersections(prepared.model, prepared.transforms, 'crowns'),
      rootPairs: intersections(prepared.model, prepared.transforms, 'roots'),
    };
    const variants = {};
    for (const variant of definition.variants) {
      const progressSamples = [
        ...new Set([...samples, ...variant.keyframes.map(item => item.progress)]),
      ].sort((a, b) => a - b);
      const frames = progressSamples.map(progress => {
        const transforms = sampleCaseDemonstration(definition.id, variant.id, progress);
        const crownPairs = intersections(prepared.model, transforms, 'crowns'),
          rootPairs = intersections(prepared.model, transforms, 'roots');
        return {
          progress,
          crownPairs,
          rootPairs,
          newCrownPairs: crownPairs.filter(
            pair => !baseline.crownPairs.some(initial => pairKey(initial) === pairKey(pair)),
          ),
          newRootPairs: rootPairs.filter(
            pair => !baseline.rootPairs.some(initial => pairKey(initial) === pairKey(pair)),
          ),
        };
      });
      ...
      console.log(
        JSON.stringify({
          case: definition.id,
          variant: variant.id,
          counts: frames.map(item => ({
            progress: item.progress,
            crowns: item.crownPairs.length,
            roots: item.rootPairs.length,
            newCrowns: item.newCrownPairs.length,
            newRoots: item.newRootPairs.length,
          })),
        }),
      );
    }
  ...
  const report = {
    ...
    limitation:
      'Not swept collision detection. Crossings between samples, enclosed volumes, crown–root pairs, gingiva, bone and biological limits are not assessed. Empty pairs do not validate a movement or bite.',
    samples,
    reference: { crownPairs: references.crowns, rootPairs: references.roots },
    cases,
  };
  if (!sourceBytes.equals(readFileSync('src/lib/teaching-cases.ts')))
    throw new Error('Teaching cases changed during the audit; rerun before publishing.');
