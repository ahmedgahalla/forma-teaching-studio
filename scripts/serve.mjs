...
const server = http.createServer((req, res) => {
  ...
  if (route === '/forma-health') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end('{"app":"forma-dental-studio","status":"ok"}');
    return;
  }
  let target = path.resolve(root, `.${route}`);
  if ((target !== root && !target.startsWith(root + path.sep)) || route.includes('\\')) {
    res.writeHead(403);
    res.end();
    return;
  }
...
server.on('error', error => {
  console.error(
    `Could not open port ${port}: ${error.message}. Close the other local server, or set PORT to another port.`,
  );
  process.exit(1);
});
