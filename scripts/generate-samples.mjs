...
const model = createOrthodonticDemo(), exporter = new STLExporter();
for (const piece of [...model.teeth, ...model.gums]) {
  const mesh = new Mesh(piece.geometry); mesh.position.fromArray(piece.position); mesh.updateMatrixWorld(true);
  const output = exporter.parse(mesh, { binary: true });
  writeFileSync(`${target}${piece.id}.stl`, new Uint8Array(output.buffer));
}
console.log(`Wrote ${model.teeth.length} separate synthetic crown STLs and ${model.gums.length} gum STLs in sample-models. Schematic roots and braces are excluded.`);
