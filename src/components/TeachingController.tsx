...
export function TeachingCommandBar({ label = 'Dental command', placeholder = 'Try “show upper jaw, hide gums, and highlight molars”', value, onChange, inputRef, suggestions }: { suggestions?: string[]; label?: string; placeholder?: string; value?: string; onChange?: (text: string) => void; inputRef?: RefObject<HTMLInputElement | null> }) {
  ...
  useEffect(() => {
    const element = bar.current?.closest<HTMLElement>('.workspace-command-dock'), workspace = element?.closest<HTMLElement>('.studio-experience');
    if (!element || !workspace) return;
    const measure = () => workspace.style.setProperty('--composer-height', `${Math.ceil(element.getBoundingClientRect().height)}px`);
    measure();
    const observer = new ResizeObserver(measure);
    observer.observe(element);
    return () => { observer.disconnect(); workspace.style.removeProperty('--composer-height'); };
  }, []);
  ...
  return <section ref={bar} className="command-section teaching-command-bar" aria-label="Voice classroom controls">
    ...
    {examples && <div className="workflow-command-examples" aria-label="Suggested commands"><div className="command-examples-heading"><strong>Try with this setup</strong><button aria-label="Close command suggestions" onClick={() => setExamples(false)}>×</button></div>{commands.slice(0, 4).map(command => <button key={command} onClick={() => { setText(command); setExamples(false); }}>{command}</button>)}</div>}
    <form className={`command-input ${capturing ? 'listening' : ''}`} onSubmit={event => { event.preventDefault(); if (text.trim()) { setExamples(false); void teaching.run(text); setText(''); } }}>
