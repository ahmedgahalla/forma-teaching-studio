...
const ThemeContext = createContext<{ theme: StudioTheme; setTheme: (theme: StudioTheme) => void }>({
  theme: 'midnight',
  setTheme: () => {},
});
...
export function StudioThemeProvider({ children }: { children: ReactNode }) {
  ...
  const setTheme = (next: StudioTheme) => {
    ...
    try {
      localStorage.setItem('forma-studio-theme', next);
    } catch {
...
export function StudioThemeToggle() {
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      onClick={() => setTheme(theme === 'midnight' ? 'clinical' : 'midnight')}
    >
      {theme === 'midnight' ? <Moon size={17} /> : <Sun size={17} />}
      <span>{theme === 'midnight' ? 'Midnight' : 'Clinical'}</span>
    </button>
  );
}
