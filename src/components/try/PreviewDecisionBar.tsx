...
export function PreviewDecisionBar({
  ...
  useEffect(() => {
    ...
    return () => {
      observer.disconnect();
      workspace.style.removeProperty('--preview-height');
    };
  }, [visible]);
  ...
  return (
    <section
      ref={bar}
      className="preview-decision-bar"
      aria-label="Movement preview decision"
      data-status={pending.collision}
    >
      <div className="preview-decision-summary" role="status" aria-live="polite" aria-atomic="true">
        ...
        <div className="preview-decision-report">
          {affectedCount !== undefined && (
            <span>
              {affectedCount} {affectedCount === 1 ? 'tooth' : 'teeth'} affected ·{' '}
            </span>
          )}
          <span>{REPORT_LABELS[pending.collision]}</span>
          <span>
            {' '}
            · {pending.checkedSteps} sampled {pending.checkedSteps === 1 ? 'position' : 'positions'}
          </span>
        ...
        {unrestricted && (
          <span className="preview-decision-unrestricted">
            Unrestricted illustration · locks still apply
          </span>
        )}
      </div>
      <div className="preview-decision-actions">
        <button
          type="button"
          className="preview-decision-modify"
          disabled={busy}
          onClick={onModify}
        >
        ...
        <button
          type="button"
          className="preview-decision-discard"
          disabled={busy}
          onClick={onDiscard}
        >
        ...
        <button
          type="button"
          className="preview-decision-apply"
