...
export function savedCommandService(value: unknown): CommandServiceConfig | null {
  ...
  try {
    ...
    return { enabled: saved.enabled, url: saved.url, provider: typeof saved.provider === 'string' && providers.includes(saved.provider) ? saved.provider : undefined };
  } catch { return null; }
}
...
export function hostedCommandService(value: unknown, origin: string): CommandServiceConfig | null {
  if (!value || typeof value !== 'object' || Object.keys(value).join() !== 'commandService') return null;
  const service = (value as Record<string, unknown>).commandService;
  if (!service || typeof service !== 'object' || Object.keys(service).sort().join() !== 'enabled,provider,url') return null;
  const config = service as Record<string, unknown>;
  if (config.enabled !== true || config.url !== 'same-origin' || typeof config.provider !== 'string' || !providers.includes(config.provider)) return null;
