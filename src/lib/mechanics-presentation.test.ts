...
describe('mechanics presentation', () => {
  ...
  it('interpolates relative to a nonzero unloaded pose', () => {
    expect(mechanicsDisplayPoses(baseline, result, 0.5)['11'].translation[0]).toBeCloseTo(2.005);
    expect(mechanicsDisplayPoses(baseline, result, 0.5)['11'].rotation[1]).toBeCloseTo(10.5);
  });
  ...
  it('automatically makes a small response visible without changing its values', () => {
    const size = { maxDisplacementMm: 0.025, maxRotationDeg: 0.2 };
    const scale = recommendedMechanicsMagnification(size);
    expect(scale).toBe(50);
    expect(mechanicsResponseCaption(size, scale)).toBe(
      'Actual maximum: 0.0250 mm · 0.200° · visualization exaggerated 50×',
    );
    expect(size).toEqual({ maxDisplacementMm: 0.025, maxRotationDeg: 0.2 });
  });
  it('bounds the automatic scale by translation and rotation independently', () => {
    expect(
      recommendedMechanicsMagnification({ maxDisplacementMm: 0.2, maxRotationDeg: 0.01 }),
    ).toBe(5);
    expect(recommendedMechanicsMagnification({ maxDisplacementMm: 0.001, maxRotationDeg: 1 })).toBe(
      10,
    );
    expect(recommendedMechanicsMagnification({ maxDisplacementMm: 2, maxRotationDeg: 15 })).toBe(1);
    expect(recommendedMechanicsMagnification({ maxDisplacementMm: 0, maxRotationDeg: 0.1 })).toBe(
      50,
    );
  });
  it('does not invent movement for a zero response or amplify numerical noise', () => {
    ...
    shown.rotation.forEach((angle, axis) =>
      expect(angle).toBeCloseTo(baseline['11'].rotation[axis], 10),
    );
  });
  it('rejects invalid response diagnostics', () => {
    expect(() =>
      recommendedMechanicsMagnification({ maxDisplacementMm: NaN, maxRotationDeg: 0 }),
    ).toThrow();
    expect(() =>
      recommendedMechanicsMagnification({ maxDisplacementMm: 0, maxRotationDeg: -1 }),
    ).toThrow();
  });
});
