...
const necks = [0.2, 0.48, 0.77, 1.03, 1.24, 1.52, 1.8].map(angle => ({
  angle,
  height: 10,
  depth: 8,
}));
...
describe('authored gingival arch base', () => {
  it('raises tissue between tooth emergences while preserving a low cervical margin at each neck', () => {
    const geometry = createDemoGingiva(necks, 27, 34, false),
      material = new THREE.MeshBasicMaterial({ side: THREE.DoubleSide }),
      mesh = new THREE.Mesh(geometry, material);
    const emergence = surface(mesh, 0.48, 0)!,
      papilla = surface(mesh, 0.34, 0)!,
      margin = surface(mesh, 0.48, 8 * 0.35)!;
    expect(Number.isFinite(emergence)).toBe(true);
    expect(Number.isFinite(papilla)).toBe(true);
    expect(papilla - emergence).toBeGreaterThan(1.5);
    expect(margin).toBeGreaterThan(10);
    expect(margin).toBeLessThan(10.8);
  ...
  it.each([true, false])(
    'exports a finite watertight volume without degenerate triangles (upper=%s)',
    upper => {
      const geometry = createDemoGingiva(necks, 27, 34, upper),
      ...
      const a = new THREE.Vector3(),
        ...
        c = new THREE.Vector3();
      let volume = 0;
      expect([...p.array, ...geometry.getAttribute('normal').array].every(Number.isFinite)).toBe(
        true,
      );
