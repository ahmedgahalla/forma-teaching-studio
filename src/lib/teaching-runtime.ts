...
export type RuntimeState = {
  ...
  error: boolean;
  transcript: string;
  interpreter?: 'local' | 'ai';
};
export type TeachingHost<S> = {
  context(): TeachingContext;
  capture(): S;
  restore(snapshot: S): void;
  preflight(actions: TeachingAction[], fromSnapshot?: S): void;
...
export function createTeachingRuntime<S>(host: TeachingHost<S>) {
  ...
  const publish = (patch: Partial<RuntimeState>) => {
    state = { ...state, ...patch };
    if (!disposed) host.publish(state);
  };
  const interrupt = () => {
    token++;
    controller?.abort();
    ...
    if (activeEntry) {
      activeEntry.after = host.capture();
  ...
  const run = async (
    ...
    if (plan.clarification) {
      publish({ phase: 'idle', message: plan.clarification, error: true });
      return;
    }
    const first = plan.actions[0];
    if (first?.kind === 'stop' && plan.actions.length === 1) {
      cancel();
      return;
    }
    if (host.settle && !preflightDone) await abortable(host.settle(signal), signal);
    if (signal.aborted || own !== token) return;
    if (expectedRevision !== undefined && host.context().revision !== expectedRevision)
      throw new Error('The teaching context changed. Repeat the request for the current scene.');
    if (
      first?.kind === 'history' ||
      (first?.kind === 'dental' && (first.command.type === 'undo' || first.command.type === 'redo'))
    ) {
      host.pause();
      const redo =
        first.kind === 'history' ? first.action === 'redo' : first.command.type === 'redo';
      const count = first.kind === 'history' ? first.count : 1,
        source = redo ? future : past;
      if (first.kind === 'history' && source.length < count)
        throw new Error(
          `Only ${source.length} complete requests are available to ${redo ? 'redo' : 'undo'}; nothing was changed.`,
        );
      if (!source.length) {
        if (host.context().mode === 'case' && first.kind === 'dental') {
          host.preflight([first]);
          await abortable(Promise.resolve(host.apply(first, signal)), signal);
          if (signal.aborted || own !== token) return;
          publish({
            phase: 'idle',
            message: `${redo ? 'Redo' : 'Undo'} applied to the case movement history.`,
            error: false,
          });
        } else
          publish({
            phase: 'idle',
            message: `Nothing to ${redo ? 'redo' : 'undo'} in the command history.`,
            error: false,
          });
        return;
      }
      const entries = source.slice(-count).reverse(),
        entry = entries[entries.length - 1];
      if (redo && entry.after !== undefined) host.restore(entry.after);
      else host.restore(entry.before);
      source.splice(-count);
      (redo ? past : future).push(...entries);
      last = past.findLast(item => item.completed);
      publish({
        phase: 'idle',
        message:
          count === 1
            ? `${redo ? 'Restored' : 'Undid'} the whole request: ${entry.label}`
            : `${redo ? 'Restored' : 'Undid'} ${count} complete requests.`,
        error: false,
      });
      return;
    }
    if (first?.kind === 'replay') {
      ...
      if (previous) {
        ...
        const actions: TeachingAction[] = previous.actions.map(action =>
          speed !== undefined && action.kind === 'speed' ? { kind: 'speed', value: speed } : action,
        );
        host.preflight(actions, previous.replayStart);
        if (speed !== undefined)
          host.preflight([{ kind: 'speed', value: speed }], previous.replayStart);
        host.restore(previous.replayStart);
        await run(
          { actions, summary: `Repeat: ${previous.label}`, clarification: null },
          own,
          signal,
          current,
          true,
          previous.replayStart,
          speed,
        );
        return;
      }
      plan = {
        ...plan,
        actions: [
          ...(first.slower ? [{ kind: 'speed', value: 0.5 } as TeachingAction] : []),
    ...
    const entry: Entry = {
      before: start,
      replayStart: replayStart ?? start,
      replaySpeed,
      actions: plan.actions,
      label: plan.summary || state.transcript,
      completed: false,
    };
    activeEntry = entry;
    past.push(entry);
    ...
    publish({
      phase: 'executing',
      message: plan.summary || 'Running your instruction.',
      error: false,
    });
    ...
    try {
      ...
      if (replaySpeed !== undefined) {
        const speed: TeachingAction = { kind: 'speed', value: replaySpeed };
        await abortable(Promise.resolve(host.apply(speed, signal)), signal);
      ...
      for (const action of plan.actions) {
        ...
        if (
          ...
          if (signal.aborted || own !== token) return;
          if (['select', 'view', 'arch', 'toggle', 'anatomy', 'speed'].includes(action.kind))
            overrides.push(action);
          // A guided step has useful defaults, but explicit directions in this request win.
          if (
            (action.kind === 'workflow' && ['start', 'play'].includes(action.action)) ||
      ...
      past.pop();
      future.splice(0, future.length, ...previousFuture);
      activeEntry = undefined;
      throw error;
    }
  };
  const submit = async (text: string, options: { interpreter?: 'auto' | 'ai' } = {}) => {
    ...
    publish({
      ...
      transcript: text,
      message:
        options.interpreter === 'ai'
          ? 'Asking AI to interpret your request…'
    ...
    try {
      ...
      if (!plan || (options.interpreter === 'ai' && !localControl && !plan.clarification)) {
        ...
        plan = validateTeachingPlan(
          response,
          { ...host.context(), lastActions: last?.actions },
          { sourceText: text, expectedRevision: revision },
        );
      ...
      if (own === token && !signal.aborted)
        publish({
          phase: 'idle',
          message: error instanceof Error ? error.message : 'Could not run that instruction.',
          error: true,
        });
    }
  };
  const submitActions = async (actions: TeachingAction[], summary: string) => {
    ...
    try {
      const context = { ...host.context(), lastActions: last?.actions };
      const plan = validateTeachingPlan({ actions, summary, clarification: null }, context, {
  ...
  return {
    ...
    cancel,
    getState: () => ({ ...state }),
    getLastActions: () => last?.actions,
    clearHistory() {
      cancel('Command history cleared.');
      past.length = 0;
      future.length = 0;
      last = undefined;
    },
