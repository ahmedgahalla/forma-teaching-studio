...
type ToothMetadata = Omit<DentalTooth, 'geometry' | 'rootGeometry' | 'attachment'> & {
  crownMesh: string;
  rootMesh: string;
};
type GumMetadata = Omit<Gum, 'geometry'> & { mesh: string };
export type AnatomyMetadata = {
  version: 1;
  name: string;
  units: 'mm';
  teeth: ToothMetadata[];
...
const record = (value: unknown): Record<string, unknown> => {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error('Invalid teaching asset metadata.');
...
const vector = (value: unknown): Vec3 => {
  if (
    !Array.isArray(value) ||
    value.length !== 3 ||
    !value.every(n => typeof n === 'number' && Number.isFinite(n) && Math.abs(n) <= 250)
  )
    throw new Error('Invalid teaching asset coordinates.');
  return [...value] as Vec3;
};
const text = (value: unknown): string => {
  if (typeof value !== 'string' || !value.length || value.length > 120)
    throw new Error('Invalid teaching asset name.');
  return value;
};
...
export function validateAnatomyMetadata(raw: unknown): AnatomyMetadata {
  ...
  if (
    ...
    value.teeth.length !== 28 ||
    !Array.isArray(value.gums) ||
    value.gums.length !== 2
  )
    throw new Error(
      'The teaching asset must contain 28 teeth and two gingival meshes in millimetres.',
    );
  const teeth = value.teeth.map(item => {
    const t = record(item),
      id = text(t.id);
    if (!IDS.includes(id) || t.calibrated !== true)
      throw new Error('The teaching asset has an invalid tooth identifier or frame.');
    const tooth: ToothMetadata = {
      id,
      name: text(t.name),
      calibrated: true,
      position: vector(t.position),
      buccal: vector(t.buccal),
      mesial: vector(t.mesial),
      occlusal: vector(t.occlusal),
      bracketPosition: vector(t.bracketPosition),
      crownMesh: text(t.crownMesh),
      rootMesh: text(t.rootMesh),
    };
    anatomicalFrame(tooth);
    if (t.rootAnatomy !== undefined)
      tooth.rootAnatomy = validateRootAnatomy(t.rootAnatomy, tooth.occlusal);
    return tooth;
  });
  if (
    new Set(teeth.map(t => t.id)).size !== 28 ||
    new Set(teeth.flatMap(t => [t.crownMesh, t.rootMesh])).size !== 56
  )
    throw new Error('Teaching asset tooth and mesh names must be unique.');
  const gums = value.gums.map(item => {
    const g = record(item);
    if (g.arch !== 'upper' && g.arch !== 'lower')
      throw new Error('The gingival arch must be specified.');
    return {
      id: text(g.id),
      arch: g.arch,
      position: vector(g.position),
      mesh: text(g.mesh),
    } as GumMetadata;
  });
  if (
    new Set(gums.map(g => g.arch)).size !== 2 ||
    new Set([...teeth.flatMap(t => [t.crownMesh, t.rootMesh]), ...gums.map(g => g.mesh)]).size !==
      58
  )
    throw new Error('Teaching asset arch and mesh names must be unique.');
...
export function dentalCaseFromAsset(scene: Object3D, raw: unknown): DentalCase {
  const metadata = validateAnatomyMetadata(raw),
  ...
  const take = (name: string, position: Vec3, maxExtent: number) => {
    ...
    if (
      !vertices ||
      vertices.count < 3 ||
      vertices.count > 600_000 ||
      !Array.from(vertices.array).every(Number.isFinite)
    )
      throw new Error(`Invalid anatomy vertices: ${name}.`);
    if (
      geometry.index &&
      (geometry.index.count % 3 ||
        Array.from(geometry.index.array).some(
          i => !Number.isInteger(i) || i < 0 || i >= vertices.count,
        ))
    )
      throw new Error(`Invalid anatomy triangles: ${name}.`);
    if (!geometry.index && vertices.count % 3)
      throw new Error(`Invalid anatomy triangles: ${name}.`);
    geometry.computeBoundingBox();
    geometry.computeBoundingSphere();
    const extent = geometry.boundingBox!.getSize(new Vector3()).length();
    if (!Number.isFinite(extent) || extent < 0.1 || extent > maxExtent)
      throw new Error(`Check anatomy units: ${name}.`);
  ...
  try {
    return {
      name: metadata.name,
      demo: true,
      teeth: metadata.teeth.map(({ crownMesh, rootMesh, ...tooth }) => ({
        ...tooth,
        geometry: take(crownMesh, tooth.position, 60),
        rootGeometry: take(rootMesh, tooth.position, 60),
      })),
      gums: metadata.gums.map(({ mesh, ...gum }) => ({
        ...gum,
        geometry: take(mesh, gum.position, 200),
      })),
    };
  } catch (error) {
...
export function getTeachingAssetCase(): DentalCase | undefined {
  ...
  return {
    ...prepared,
    teeth: prepared.teeth.map(({ geometry, rootGeometry, ...metadata }) => ({
      ...structuredClone(metadata),
      geometry,
      rootGeometry,
    })),
    gums: prepared.gums.map(gum => ({ ...gum, position: [...gum.position] as Vec3 })),
  };
}
