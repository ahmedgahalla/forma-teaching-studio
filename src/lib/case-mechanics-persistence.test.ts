...
type Saved = {
  ...
  model: {
    name: string;
    demo: boolean;
    teeth: { id: string; position: Vec3; vertices: number[] }[];
  };
  transforms: Transforms;
  session: CaseSession;
};
const asFile = (data: unknown) =>
  new File([JSON.stringify(data)], 'lecture.forma.json', { type: 'application/json' });
function fixture() {
  const model: DentalCase = {
    name: 'Saved mechanics lecture',
    demo: true,
    gums: [],
    teeth: ['11', '21'].map((id, i) => ({
      ...
      rootGeometry: new BoxGeometry(0.5, 12, 0.5).translate(0, -6, 0),
      bracketPosition: [0, 0, 1.6],
      buccal: [0, 0, 1],
      mesial: [1, 0, 0],
      occlusal: [0, 1, 0],
      calibrated: true,
  ...
  models.push(model);
  const baseline: Transforms = { '11': { translation: [0.2, 0.1, 0], rotation: [0, 5, 0] } };
  ...
  mechanics = transitionMechanics(mechanics, {
    ...
    to: { kind: 'tooth', tooth: '11', local: [0, 0, 1.6] },
  ...
  mechanics = attachMechanicsResult(
    mechanics,
    solveMechanics(experimentWithoutTad(mechanics, 'tad-1')),
    true,
  );
  mechanics.applied = result;
  const session: CaseSession = {
    stages: 8,
    checkpoints: [{ id: 'baseline', name: 'Reference arrangement', transforms: baseline }],
    past: [{ label: 'Initial response', value: baseline }],
    future: [],
    braces: true,
    roots: true,
    bracketStyle: 'metal',
    ...
    attachments: false,
    tryMode: serializeTrySession({ ...createTryState(result.transforms), lockedIds: ['21'] }),
...
async function serialized() {
  vi.useFakeTimers();
  const { model, session, transforms } = fixture();
...
async function restored(data: unknown) {
  const result = await loadCase(asFile(data));
...
describe('v3 mechanics and lecture case persistence', () => {
  it('round trips all appliance families, stages, lecture view and actual committed poses', async () => {
    const { raw, session, transforms, model } = await serialized();
    expect(raw.version).toBe(3);
    expect(raw.units).toBe('mm');
    const loaded = await restored(raw),
      mechanics = loaded.session!.mechanics!;
    expect(loaded.transforms).toEqual(transforms);
    expect(loaded.session!.lectureSetup).toEqual(session.lectureSetup);
    expect(loaded.session!.tryMode).toEqual(session.tryMode);
    expect(loaded.session!.checkpoints).toEqual(session.checkpoints);
    expect(loaded.session!.past).toEqual(session.past);
    expect(mechanics.config).toEqual(session.mechanics!.config);
    expect(mechanics.reference).toEqual(session.mechanics!.reference);
    expect(mechanics.stages).toEqual(session.mechanics!.stages);
    expect(mechanics.stageIndex).toBe(0);
    expect(mechanics.result).toBeNull();
    expect(mechanics.applied).toBeNull();
    expect(mechanics.comparison).toBeNull();
    expect(solveMechanics(mechanics).transforms).toEqual(transforms);
    loaded.model.teeth.forEach((tooth, index) => {
      expect(Array.from(tooth.geometry.getAttribute('position').array)).toEqual(
        Array.from(model.teeth[index].geometry.getAttribute('position').array),
      );
      expect(Array.from(tooth.rootGeometry!.getAttribute('position').array)).toEqual(
        Array.from(model.teeth[index].rootGeometry!.getAttribute('position').array),
      );
    });
  });
  ...
  it('valid changed inputs invalidate the previous response rather than trusting saved output', async () => {
    const { raw, transforms } = await serialized();
    raw.session.mechanics!.config.elastics[0].law = { kind: 'constant', forceN: 0.2 };
    const loaded = await restored(raw);
  ...
  it('normalizes missing response playback flags from earlier v3 files without trusting a cached result', async () => {
    const { raw } = await serialized(),
      lecture = raw.session.lectureSetup!;
    for (const key of [
      'mechanicsResponse',
      'responseRevealed',
      'predictResponse',
      ...
      delete lecture[key];
    const loaded = await restored(raw);
    expect(loaded.session!.lectureSetup).toMatchObject({
      stage: 3.5,
      magnification: 10,
  ...
  it.each([0, 0.25, 1.5, 3, NaN, Infinity, null, '1'])(
    ...
    value => {
      const { session } = fixture(),
        invalid = { ...session.lectureSetup, playbackSpeed: value };
      expect(() => validateLectureSetup(invalid, new Set(['11', '21']), 8)).toThrow(
        /response playback/,
      );
    },
  );
  it.each([0.5, 1, 2])(
    'preserves supported playback speed %s and explicit false switches',
    speed => {
      const { session } = fixture();
      expect(
        validateLectureSetup(
          {
  ...
  it('rejects absent appliance targets and a reference that does not match the saved model', async () => {
    const { raw } = await serialized(),
      absent = structuredClone(raw);
    absent.session.mechanics!.config.wires[0].teeth.push('31');
    await expect(restored(absent)).rejects.toThrow(/teeth present/);
    const movedModel = structuredClone(raw);
    movedModel.model.teeth[0].position[0] += 1;
    await expect(restored(movedModel)).rejects.toThrow(/reference does not match/);
    const invalid = structuredClone(raw);
    invalid.session.mechanics!.config.tads[0].position[0] = Infinity;
    await expect(restored(invalid)).rejects.toThrow(/finite mechanics/);
  });
  it.each(['magnification', 'stage', 'camera', 'selection', 'section'] as const)(
    'rejects invalid saved lecture %s',
    async field => {
      const { raw } = await serialized(),
        lecture = raw.session.lectureSetup!;
      ...
      if (field === 'section') lecture.wirePreset.section = { shape: 'round', diameterMm: 0.7 };
      await expect(restored(raw)).rejects.toThrow();
    },
  );
  it.each([1, 2])(
    'keeps version %s cases without mechanics or lecture metadata compatible',
    async version => {
      const { raw, transforms } = await serialized();
      raw.version = version;
      delete raw.session.mechanics;
      delete raw.session.lectureSetup;
      if (version === 1) Reflect.deleteProperty(raw, 'session');
      const loaded = await restored(raw);
      expect(loaded.transforms).toEqual(transforms);
      expect(loaded.session?.mechanics).toBeUndefined();
      expect(loaded.session?.lectureSetup).toBeUndefined();
