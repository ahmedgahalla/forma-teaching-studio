...
function signedVolume(geometry: THREE.BufferGeometry) {
  ...
  for (let i = 0; i < index.count; i += 3) {
    a.fromBufferAttribute(points, index.getX(i));
    b.fromBufferAttribute(points, index.getX(i + 1));
    c.fromBufferAttribute(points, index.getX(i + 2));
...
function axialRange(tooth: DentalTooth) {
  const points = tooth.geometry.getAttribute('position'),
    axis = new THREE.Vector3(...tooth.occlusal!);
  const values = Array.from({ length: points.count }, (_, i) =>
    new THREE.Vector3().fromBufferAttribute(points, i).dot(axis),
  );
...
function surfacePoint(
  ...
  mesial: number,
  buccal: number,
  direction: 'buccal' | 'occlusal',
  root = false,
) {
  const axes = {
    buccal: new THREE.Vector3(...tooth.buccal),
    occlusal: new THREE.Vector3(...tooth.occlusal!),
    mesial: new THREE.Vector3(...tooth.mesial),
  };
  const origin = axes.occlusal
    .clone()
    .multiplyScalar(axial)
    .addScaledVector(axes.mesial, mesial)
    .addScaledVector(axes.buccal, buccal);
  const material = new THREE.MeshBasicMaterial();
  const hit = new THREE.Raycaster(origin, axes[direction].clone().negate()).intersectObject(
    new THREE.Mesh(root ? tooth.rootGeometry! : tooth.geometry, material),
  )[0];
  material.dispose();
...
describe('schematic orthodontic demo', () => {
  it('contains 28 uniquely numbered permanent teeth, both gingival arches and no wisdom teeth', () => {
    ...
    for (const quadrant of [1, 2, 3, 4]) {
      expect(model.teeth.filter(t => t.id.startsWith(String(quadrant))).map(t => t.id)).toEqual(
    ...
    for (const tooth of model.teeth)
      for (const geometry of [tooth.geometry, tooth.rootGeometry!]) {
        ...
        for (let i = 0; i < index.count; i += 3) {
          ...
          a.fromBufferAttribute(points, ids[0]);
          b.fromBufferAttribute(points, ids[1]);
          c.fromBufferAttribute(points, ids[2]);
          minTriangleAreaSquared = Math.min(
            minTriangleAreaSquared,
            b.sub(a).cross(c.sub(a)).lengthSq(),
          );
          for (let e = 0; e < 3; e++) {
            const key = [ids[e], ids[(e + 1) % 3]].sort((a, b) => a - b).join(':');
  ...
  it('centers each pivot on the crown and registers schematic roots in that same frame', () => {
    for (const tooth of model.teeth) {
      ...
      const crownBox = bounds(tooth.geometry, tooth.position),
        rootBox = bounds(tooth.rootGeometry!, tooth.position);
      if (upper(tooth.id)) {
        expect(rootBox.max.y).toBeGreaterThan(crownBox.max.y + 10);
        expect(Math.abs(rootBox.min.y - crownBox.max.y)).toBeLessThan(0.2);
      } else {
        expect(rootBox.min.y).toBeLessThan(crownBox.min.y - 10);
        expect(Math.abs(rootBox.max.y - crownBox.min.y)).toBeLessThan(0.2);
      }
  ...
  it('uses calibrated orthonormal anatomical directions and the correct arch-specific occlusal sign', () => {
    for (const tooth of model.teeth) {
      ...
      const vectors = [tooth.buccal, tooth.mesial, tooth.occlusal!].map(
  ...
  it('separates adjacent crowns instead of interpenetrating on the arch bend', () => {
    const pairs = model.teeth
      .filter(t => t.id[1] !== '7')
      .map(t => [t, model.teeth.find(n => Number(n.id) === Number(t.id) + 1)!]);
    pairs.push(['11', '21'].map(id => model.teeth.find(t => t.id === id)!));
    pairs.push(['31', '41'].map(id => model.teeth.find(t => t.id === id)!));
    for (const [a, b] of pairs) {
      const axis = new THREE.Vector3(...b.position).sub(new THREE.Vector3(...a.position));
      axis.y = 0;
      axis.normalize();
      ...
      expect(project(b)[0] - project(a)[1]).toBeGreaterThan(0.01);
    }
  });
  ...
  it('has distinct incisal blades, rounded canine ridges and multi-cusp posterior surfaces', () => {
    ...
    expect(bounds(incisor.geometry).getSize(new THREE.Vector3()).x).toBeGreaterThan(
      bounds(lowerIncisor.geometry).getSize(new THREE.Vector3()).x + 2,
    );
    const height = (tooth: DentalTooth, x: number, z: number) =>
      surfacePoint(tooth, 30, x, z, 'occlusal').dot(new THREE.Vector3(...tooth.occlusal!));
    const canine = model.teeth.find(t => t.id === '13')!;
    expect(height(incisor, 0, 0) - height(incisor, 2.5, 0)).toBeLessThan(0.4);
    ...
    expect(height(premolar, 0, 1.6) - premolarFossa).toBeGreaterThan(0.4);
    expect(height(premolar, 0, -1.6) - premolarFossa).toBeGreaterThan(0.25);
    for (const id of ['16', '36']) {
      const tooth = model.teeth.find(t => t.id === id)!;
      const fossa = height(tooth, 0, 0);
      for (const x of [-1.8, 1.8])
        for (const z of [-1.8, 1.8]) expect(height(tooth, x, z) - fossa).toBeGreaterThan(0.4);
    }
  });
  ...
  it('keeps a single convex sweep across the visible anterior labial face without horizontal profile shelves', () => {
    for (const id of ['11', '12', '13', '31', '32', '33']) {
      const tooth = model.teeth.find(t => t.id === id)!,
        [low, high] = axialRange(tooth),
        buccal = new THREE.Vector3(...tooth.buccal);
      const depths = Array.from({ length: 7 }, (_, i) =>
        surfacePoint(tooth, low + (high - low) * (0.3 + i * 0.05), 0, 30, 'buccal').dot(buccal),
      );
      const slopes = depths.slice(1).map((value, i) => value - depths[i]);
      for (let i = 1; i < slopes.length; i++)
        expect(slopes[i] - slopes[i - 1], `Labial shelf on ${id}`).toBeLessThan(-0.006);
    }
  });
  ...
  it('joins single roots to the cervical crown contour and terminates in rounded apices', () => {
    for (const id of ['11', '13', '15', '31', '33', '35']) {
      const tooth = model.teeth.find(t => t.id === id)!,
        [cervical] = axialRange(tooth),
        buccal = new THREE.Vector3(...tooth.buccal);
      const crown = surfacePoint(tooth, cervical + 0.03, 0, 30, 'buccal');
      const root = surfacePoint(tooth, cervical - 0.03, 0, 30, 'buccal', true);
      expect(Math.abs(crown.clone().sub(root).dot(buccal))).toBeLessThan(0.18);
      const points = tooth.rootGeometry!.getAttribute('position'),
        normals = tooth.rootGeometry!.getAttribute('normal');
      const rootward = new THREE.Vector3(...tooth.occlusal!).negate();
      const axial = Array.from({ length: points.count }, (_, i) =>
        new THREE.Vector3().fromBufferAttribute(points, i).dot(rootward),
      );
      const apex = Math.max(...axial),
        tips = axial.map((value, i) => ({ value, i })).filter(p => apex - p.value < 1e-5);
      expect(tips).toHaveLength(1); // No planar disk across the end of a root cone.
      expect(
        new THREE.Vector3().fromBufferAttribute(normals, tips[0].i).dot(rootward),
  ...
  it('keeps finite closed gingiva outside the rest gap', () => {
    for (const gum of model.gums) {
      expect(Array.from(gum.geometry.getAttribute('position').array).every(Number.isFinite)).toBe(
        true,
      );
      expect(signedVolume(gum.geometry)).toBeGreaterThan(100);
      const index = gum.geometry.index!,
