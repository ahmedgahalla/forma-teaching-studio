...
export function validateRootAnatomy(raw: unknown, occlusal?: Vec3): RootAnatomy {
  const fail = (): never => { throw new Error('Invalid synthetic root anatomy metadata.'); };
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return fail();
  const value = raw as Record<string, unknown>;
  if (value.version !== 1 || !Array.isArray(value.branches) || value.branches.length < 1 || value.branches.length > 3) return fail();
  const sections = (input: unknown): RootSection[] => {
    ...
    const result = input.map(item => {
      ...
      if (!Array.isArray(center) || center.length !== 3 || !center.every(n => typeof n === 'number' && Number.isFinite(n) && Math.abs(n) <= 80)) return fail();
      if (!Array.isArray(radii) || radii.length !== 2 || !radii.every(n => typeof n === 'number' && Number.isFinite(n) && n >= .015 && n <= 12)) return fail();
    ...
    for (let i = 1; i < result.length; i++) {
      ...
      if (distance < .00001 || distance > 15 || (occlusal && difference.reduce((sum, n, k) => sum - n * occlusal[k], 0) <= .00001)) return fail();
    }
    return result;
  };
  return { version: 1, ...(value.trunk !== undefined ? { trunk: sections(value.trunk) } : {}), branches: value.branches.map(sections) };
}
