...
function requirePose(pose: Pose): void {
  if (!isPose(pose)) throw new Error("A pose must contain three finite translation and rotation values.");
}
...
function scaledUnit(vector: Vec3, amount: number): Vec3 {
  if (!isVec3(vector)) throw new Error("This tooth has an invalid anatomical direction. Use X, Y, or Z.");
...
export function anatomicalFrame(tooth: Tooth): { buccal: Vec3; mesial: Vec3; occlusal: Vec3 } {
  if (!tooth.calibrated) {
    throw new Error("Anatomical movement requires a calibrated tooth frame. Use X, Y, or Z for this import.");
  }
  ...
  if ([dot(buccal, mesial), dot(buccal, occlusal), dot(mesial, occlusal)].some(value => Math.abs(value) > 0.001)) {
    throw new Error("The calibrated anatomical axes must be perpendicular. Recalibrate this tooth or use X, Y, or Z.");
  }
...
export function resolveMovement(
  ...
  switch (direction) {
    case "buccal": return scaledUnit(frame.buccal, amount);
    case "lingual": return scaledUnit(frame.buccal, -amount);
    case "mesial": return scaledUnit(frame.mesial, amount);
    case "distal": return scaledUnit(frame.mesial, -amount);
    case "intrude": return scaledUnit(frame.occlusal, -amount);
    case "extrude": return scaledUnit(frame.occlusal, amount);
...
export function addMovement(pose: Pose, delta: Vec3): Pose {
  ...
  return {
    translation: pose.translation.map((value, i) => finite(finite(value) + finite(delta[i]))) as Vec3,
...
function composeRotation(pose: Pose, axis: Vec3, amount: number): Pose {
  requirePose(pose);
  finite(amount);
  const current = new Quaternion().setFromEuler(new Euler(...pose.rotation.map(MathUtils.degToRad) as Vec3, "XYZ"));
  const increment = new Quaternion().setFromAxisAngle(new Vector3(...axis), MathUtils.degToRad(amount));
  current.premultiply(increment).normalize();
  const euler = new Euler().setFromQuaternion(current, "XYZ");
  return {
    translation: [...pose.translation],
    rotation: [euler.x, euler.y, euler.z].map(value => finite(MathUtils.radToDeg(value)) || 0) as Vec3,
  };
}
 ...
 */
export function addOrthodonticRotation(pose: Pose, tooth: Tooth, movement: "tip" | "torque" | "rotate", amount: number): Pose {
  const frame = anatomicalFrame(tooth);
  const axis = movement === "tip" ? frame.buccal : movement === "torque" ? frame.mesial : movement === "rotate" ? frame.occlusal : null;
...
export function applyDentalCommand(transforms: Transforms, teeth: readonly Tooth[], command: Command): Transforms {
  if (!["move", "move_group", "rotate", "rotate_group", "orthodontic", "reset"].includes(command.type)) return transforms;
  ...
  for (const id of ids) {
    ...
    switch (command.type) {
      case "move": case "move_group": updated[id] = addMovement(pose, resolveMovement(tooth, command.direction, command.amount)); break;
      case "rotate": case "rotate_group": updated[id] = addRotation(pose, command.axis, command.amount); break;
      case "orthodontic": updated[id] = addOrthodonticRotation(pose, tooth, command.movement, command.amount); break;
      case "reset": updated[id] = emptyPose(); break;
    }
  }
  return updated;
}
