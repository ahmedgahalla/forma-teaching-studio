...
type VariantAudit = { sampleCount: number; frames: { progress: number; crownPairs: Pair[]; rootPairs: Pair[] }[] };
...
export function casePathAudit(caseId: string, variantId: string) {
  ...
  for (const frame of result.frames) for (const tissue of ['crown', 'root'] as const) {
    for (const pair of frame[`${tissue}Pairs`]) pairs.set(`${tissue}/${pair.a}/${pair.b}`, { ...pair, tissue });
  }
