...
export type WireSection =
  ...
  | { shape: 'rectangle'; widthMm: number; heightMm: number };
export type MechanicsEndpoint =
  { kind: 'tooth'; tooth: string; local: Vec3 } | { kind: 'tad'; id: string };
...
export type MechanicsExperiment = {
  version: 1;
  revision: number;
  reference: { teeth: MechanicsTooth[]; transforms: Transforms };
...
export type MechanicsResult = {
  ...
  teeth: ToothMechanicsResult[];
  wires: { id: string; points: Vec3[]; maxStrain: number }[];
  elastics: MechanicsLine[];
  expanders: (MechanicsLine & {
    dentalOpeningMm: number;
    skeletalOpeningMm: number;
    applianceDeflectionMm: number;
  })[];
...
export type MechanicsWorkerResponse =
  { requestId: number; result: MechanicsResult } | { requestId: number; error: string };
