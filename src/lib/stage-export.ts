...
function triangleCount(geometry: THREE.BufferGeometry): number {
  ...
  if (!Number.isSafeInteger(count) || count < 1) throw new Error('Cannot export an invalid triangle mesh.');
  return count;
}
...
function requireBatchSize(bytesPerStage: number, stages: number) {
  if (!Number.isSafeInteger(bytesPerStage) || bytesPerStage * stages > MAX_BATCH_BYTES) {
    throw new Error('The stage sequence exceeds 250 MB of raw STL data. Reduce the stage count or mesh size, or export individual stages.');
  }
}
...
function prepareScene(model: DentalCase, includeAttachments: boolean, stages = 1) {
  const baseTriangles = [...model.teeth, ...model.gums].reduce((count, part) => count + triangleCount(part.geometry), 0);
  ...
  const dispose = () => { attachments.forEach(geometry => geometry.dispose()); material.dispose(); };
  try {
    for (const entry of teeth) {
      ...
      if (includeAttachments && entry.tooth.attachment) {
        ...
        attachments.push(geometry); entry.group.add(new THREE.Mesh(geometry, material));
      }
      group.add(entry.group);
    }
    for (const gum of model.gums) {
      const mesh = new THREE.Mesh(gum.geometry, material); mesh.position.fromArray(gum.position); group.add(mesh);
    }
    const bytesPerStage = 84 + (baseTriangles + attachments.reduce((sum, geometry) => sum + triangleCount(geometry), 0)) * 50;
    requireBatchSize(bytesPerStage, stages);
    return { group, teeth, dispose, bytesPerStage };
  } catch (error) { dispose(); throw error; }
}

function encodeStage(scene: ReturnType<typeof prepareScene>, transforms: Transforms): Uint8Array<ArrayBuffer> {
  for (const { tooth, group } of scene.teeth) {
    ...
    group.rotation.set(...(pose?.rotation || [0, 0, 0]).map(THREE.MathUtils.degToRad) as Vec3, 'XYZ');
  }
  ...
  bytes.set(strToU8('Forma educational geometric stage; units mm; not for clinical manufacture.').subarray(0, 80));
  return bytes;
}

/** Crown surfaces and static gums, with optional separate attachment surfaces; no Boolean union. */
export function buildStageSTL(model: DentalCase, transforms: Transforms, includeAttachments: boolean): Uint8Array<ArrayBuffer> {
  requireTransforms(model, transforms);
  const scene = prepareScene(model, includeAttachments);
  try { return encodeStage(scene, transforms); } finally { scene.dispose(); }
}

export function exportStage(model: DentalCase, transforms: Transforms, stageIndex: number, includeAttachments: boolean): void {
  if (!Number.isInteger(stageIndex) || stageIndex < 0 || stageIndex > 50) throw new Error('Stage index must be an integer between 0 and 50.');
  download(`forma-${stageName(stageIndex)}`, buildStageSTL(model, transforms, includeAttachments), 'model/stl');
}
...
export async function exportStageSequence(model: DentalCase, final: Transforms, checkpoints: Checkpoint[], count: number, includeAttachments: boolean, trajectory?: TryPreview | null, initialResponse?: InitialResponsePath, original: Transforms = {}): Promise<void> {
  ...
  const archive = await new Promise<Uint8Array<ArrayBuffer>>((resolve, reject) => {
    zipAsync(files, { level: 1, consume: true }, (error, data) => error ? reject(error) : resolve(data));
  });
