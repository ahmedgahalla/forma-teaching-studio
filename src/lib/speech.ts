...
const errorMessages: Record<string, string> = {
  'not-allowed':
    'Microphone access was denied. Allow microphone access in the browser, then start listening again.',
  'service-not-allowed':
    'The browser did not allow its speech service. Check browser permissions, then start listening again.',
  'audio-capture':
    'The microphone is unavailable. Check its connection and browser permissions, then start listening again.',
  network:
    'The browser speech service could not connect. Check your connection, then start listening again.',
...
export function createSpeechController(
  ...
  const publish = (patch: Partial<SpeechState>) => {
    ...
    if (
      ...
      return;
    state = next;
    if (!disposed) callbacks.onState({ ...state });
  };
  const detach = (recognition: SpeechRecognitionLike) => {
  ...
  const cancel = () => {
    ...
    if (recognition) {
      ...
      try {
        ...
        /* An already-ended browser session needs no further cleanup. */
      }
    }
  ...
  function begin() {
    ...
    try {
      ...
      fail(
        'This browser could not initialize speech recognition. Type a command or try another supported browser.',
      );
      return;
    }
    active = recognition;
    ...
    recognition.onstart = () => {
      if (current()) publish({ listening: true });
    };
    ...
    recognition.onerror = event => {
      ...
      fail(
        errorMessages[event.error] ||
          'The browser speech service stopped. Start listening again or type your command.',
      );
    };
    recognition.onend = () => {
      if (!current()) return;
    ...
    try {
      ...
      fail(
        'The browser could not start speech recognition. Check microphone access, then try again.',
      );
    }
  }
  ...
  return {
    ...
    start(nextContinuous: boolean) {
      ...
      if (!Recognition) {
        callbacks.onError(
          'Speech recognition is unavailable in this browser. Type a command or use a supported browser.',
        );
        return;
      }
      ...
      wanted = true;
      lastFinal = '';
      publish({ listening: true, transcript: '' });
      begin();
    },
    stop: cancel,
    dispose() {
