...
describe('fixed-appliance display geometry', () => {
  it('attaches every demo bracket to its buccal surface with a right-handed anatomical orientation', () => {
    ...
    for (const tooth of model.teeth) {
      ...
      expect(
        new THREE.Vector3(0, 0, 1).applyQuaternion(bracket.quaternion).distanceTo(buccal),
      ).toBeLessThan(1e-10);
      expect(
        new THREE.Vector3(0, 1, 0).applyQuaternion(bracket.quaternion).distanceTo(occlusal),
      ).toBeLessThan(1e-10);
      ...
      const ray = new THREE.Raycaster(
        bracket.position.clone().addScaledVector(buccal, 10),
        buccal.clone().negate(),
      );
      ...
      expect(attachmentGap, `Bracket floats away from ${tooth.id}`).toBeLessThan(0.275);
  ...
  it('orders partial and complete arches continuously from patient right posterior through the midline', () => {
    const ids = model.teeth.map(t => t.id),
      original = [...ids];
    expect(orderedArchIds(ids, 'upper')).toEqual([
      '17',
      '16',
      '15',
      '14',
    ...
    expect(orderedArchIds(ids, 'lower')).toEqual([
      '47',
      '46',
      '45',
      '44',
    ...
    expect(orderedArchIds(['23', '11', '31', '16', '21', '41'], 'upper')).toEqual([
      '16',
      '11',
      '21',
      '23',
  ...
  it('withholds appliances until an imported tooth has a calibrated frame', () => {
    ...
    expect(new THREE.Vector3(0, 0, 1).applyQuaternion(bracket.quaternion).toArray()).toEqual([
      0, 0, 1,
    ]);
    geometry.dispose();
    kit.dispose();
  ...
  it('changes the bracket finish and ligature colour while retaining the metallic wire and recessed slot', () => {
    const kit = createApplianceKit(),
      bracket = kit.bracket(model.teeth[0])!;
    const meshes = bracket.children as THREE.Mesh[];
    const slotMaterial = meshes[5].material,
      initialMetal = meshes[0].material as THREE.MeshStandardMaterial,
      initialWire = kit.wireMaterial;
    expect(initialWire).not.toBe(initialMetal);
    expect(initialWire.metalness).toBeGreaterThan(0.8);
  ...
  it('shares and disposes appliance mesh resources once across all tooth attachments', () => {
    ...
    const geometries = new Set(
      brackets.flatMap(bracket => (bracket.children as THREE.Mesh[]).map(mesh => mesh.geometry)),
    );
    ...
    kit.dispose();
    [...disposeSpies, wireDispose].forEach(spy => expect(spy).toHaveBeenCalledOnce());
  });
  ...
  it('shows a narrow tie-wing neck while preserving the wire channel and seven-part bracket', () => {
    ...
    const widthAt = (predicate: (z: number) => boolean) => {
      const xs = Array.from({ length: positions.count }, (_, i) => i)
        .filter(i => predicate(positions.getZ(i)))
        .map(i => positions.getX(i));
      return Math.max(...xs) - Math.min(...xs);
    };
    expect(widthAt(z => z < -0.2)).toBeLessThan(widthAt(z => z > 0.2) * 0.8);
    for (const wing of wings) {
      wing.geometry.computeBoundingBox();
      const bounds = wing.geometry.boundingBox!.clone().translate(wing.position);
      // The 0.4 mm round display wire remains clear of both occlusal and gingival wings.
      expect(bounds.min.y > 0.2 || bounds.max.y < -0.2).toBe(true);
      expect(Array.from(wing.geometry.getAttribute('normal').array).every(Number.isFinite)).toBe(
        true,
      );
    }
    kit.dispose();
