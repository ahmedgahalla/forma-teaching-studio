...
describe('bounded everyday classroom wording', () => {
  ...
  it('normalizes casual appliance arch targets without depending on the displayed arch', () => {
    ...
    const context: TeachingContext = { mode: 'case', workflowId: null, stepIndex: 0, selected: '31', selectedIds: ['31'], availableIds: ids, synthetic: true, revision: 1, view: 'occlusal', arch: 'lower', speed: 1,
      mechanics: { config: { brackets: {}, wires: [], tads: [], elastics: [], expanders: [], support: 'standard', fixedTeeth: [] }, bracketAnchors: Object.fromEntries(ids.map(id => [id, [0, 0, 3]])), focus: {}, stageIndex: -1, stageCount: 0, hasResult: false } };
    for (const sourceText of ['put brackets in top', 'install brackets on the top teeth']) {
      ...
      expect(plan.actions).toEqual([{ kind: 'mechanics', action: { type: 'brackets', teeth: ['11', '21'], installed: true } }]);
      expect(validateTeachingPlan(plan, context, { sourceText }).actions).toEqual(plan.actions);
      const wrong = { ...plan, actions: [{ kind: 'mechanics', action: { type: 'brackets', teeth: ['31', '41'], installed: true } }] };
      expect(() => validateTeachingPlan(wrong, context, { sourceText })).toThrow(/targets/);
    }
    expect(parseTeachingPlan('put brackets in bottom', context).actions).toEqual([{ kind: 'mechanics', action: { type: 'brackets', teeth: ['31', '41'], installed: true } }]);
  });
