...
export type PlanAction =
  ...
  | { type: 'load'; value: Transforms; past?: HistoryEntry[]; future?: HistoryEntry[] }
  | { type: 'undo' | 'redo' };
...
export type LectureSetup = {
  camera: {
    ...
    target: Vec3;
    up: Vec3;
    view: 'perspective' | 'front' | 'occlusal' | 'left' | 'right';
    far: number;
    maxDistance: number;
  } | null;
  selectedIds: string[];
...
export type CaseSession = {
  ...
  lectureSetup?: LectureSetup;
};

export function validateLectureSetup(raw: unknown, ids: Set<string>, stages: number): LectureSetup {
  const v = raw as LectureSetup,
    views = ['perspective', 'front', 'occlusal', 'left', 'right'];
  const finite = (n: unknown, min: number, max: number): n is number =>
    typeof n === 'number' && Number.isFinite(n) && n >= min && n <= max;
  const point = (p: unknown): p is Vec3 =>
    Array.isArray(p) && p.length === 3 && p.every(n => finite(n, -1e5, 1e5));
  if (
    !v ||
    typeof v !== 'object' ||
    !Array.isArray(v.selectedIds) ||
    !v.selectedIds.length ||
    new Set(v.selectedIds).size !== v.selectedIds.length ||
    v.selectedIds.some(id => !ids.has(id)) ||
    !['upper', 'lower', 'both'].includes(v.arch) ||
    !views.includes(v.view) ||
    ![v.gums, v.labels, v.grid, v.forceVectors].every(n => typeof n === 'boolean') ||
    !finite(v.stage, 0, stages) ||
    !finite(v.opening, 0, 25) ||
    ![1, 5, 10, 25, 50].includes(v.magnification)
  )
    throw new Error('Invalid saved lecture setup.');
  if (
    !v.anatomy ||
    ![v.anatomy.bone, v.anatomy.cutaway, v.anatomy.ligament].every(n => typeof n === 'boolean') ||
    !finite(v.anatomy.opacity, 0, 1)
  )
    throw new Error('Invalid saved anatomy visibility.');
  if (
    v.camera !== null &&
    (!v.camera ||
      ![v.camera.position, v.camera.target, v.camera.up].every(point) ||
      !views.includes(v.camera.view) ||
      !finite(v.camera.far, 1, 1e6) ||
      !finite(v.camera.maxDistance, 1, 1e6) ||
      Math.hypot(...v.camera.up) < 0.1)
  )
    throw new Error('Invalid saved lecture camera.');
  ...
  return {
    ...
    playbackSpeed: v.playbackSpeed ?? 1,
    reverse: v.reverse ?? false,
    wirePreset: {
      material: v.wirePreset.material,
      section: validateWireSection(v.wirePreset.section),
    },
  };
}
...
export function historyReducer(state: Plan, action: PlanAction): Plan {
  if (action.type === 'load')
    return { current: action.value, past: action.past || [], future: action.future || [] };
  if (action.type === 'commit')
    return {
      current: action.value,
      past: [...state.past, { value: state.current, label: action.label }],
      future: [],
    };
  if (action.type === 'undo') {
    ...
    return last
      ? {
          ...
          future: [...state.future, { value: state.current, label: last.label }],
        }
      : state;
  }
  ...
  return next
    ? {
        ...
        past: [...state.past, { value: state.current, label: next.label }],
      }
    : state;
}
...
export function interpolateTransforms(
  ...
  if (!Number.isFinite(fraction) || fraction < 0 || fraction > 1)
    throw new Error('Preview fraction must be between 0 and 1.');
  ...
  for (const id of new Set([...Object.keys(from), ...Object.keys(to)])) {
    const a = from[id] || emptyPose(),
      b = to[id] || emptyPose();
    const qa = new Quaternion().setFromEuler(
      new Euler(...(a.rotation.map(MathUtils.degToRad) as Vec3)),
    );
    const qb = new Quaternion().setFromEuler(
      new Euler(...(b.rotation.map(MathUtils.degToRad) as Vec3)),
    );
    const euler = new Euler().setFromQuaternion(qa.slerp(qb, fraction), 'XYZ');
    result[id] = {
      translation: a.translation.map(
        (value, i) => value + (b.translation[i] - value) * fraction,
      ) as Vec3,
...
export function stageTransforms(
  final: Transforms,
  checkpoints: Checkpoint[],
  stage: number,
  count: number,
  ...
  return (
    ...
    Object.entries(value).every(
      ([id, pose]) =>
        ids.has(id) &&
        isPose(pose) &&
        [...pose.translation, ...pose.rotation].every(n => Math.abs(n) <= 100000),
    )
  );
}
...
export function validateSession(value: unknown, ids: Set<string>): CaseSession | undefined {
  ...
  if (s?.attachments !== undefined && typeof s.attachments !== 'boolean')
    throw new Error('Invalid saved attachment visibility.');
  const history = (v: unknown): v is HistoryEntry[] =>
    Array.isArray(v) &&
    v.length <= 5000 &&
    v.every(
      e =>
        !!e && typeof e.label === 'string' && e.label.length < 300 && validTransforms(e.value, ids),
    );
  if (
    !s ||
    !Number.isInteger(s.stages) ||
    s.stages < 2 ||
    s.stages > 50 ||
    !Array.isArray(s.checkpoints) ||
    s.checkpoints.length > 20 ||
    !s.checkpoints.every(
      c =>
        !!c &&
        typeof c.id === 'string' &&
        ...
        c.name.length <= 60 &&
        validTransforms(c.transforms, ids),
    ) ||
    !history(s.past) ||
    !history(s.future) ||
    typeof s.braces !== 'boolean' ||
    typeof s.roots !== 'boolean' ||
    !['metal', 'ceramic'].includes(s.bracketStyle) ||
    !/^#[0-9a-f]{6}$/i.test(s.ligatureColor)
  )
    throw new Error('Invalid saved planning session.');
  if (new Set(s.checkpoints.map(c => c.id)).size !== s.checkpoints.length)
    throw new Error('Duplicate checkpoint identifiers.');
  return {
    ...s,
    ...(s.tryMode === undefined ? {} : { tryMode: validateTrySession(s.tryMode, [...ids]) }),
    ...(s.applianceDisplay === undefined
      ? {}
      : { applianceDisplay: validateApplianceDisplay(s.applianceDisplay) }),
    ...(s.lectureSetup === undefined
      ? {}
      : { lectureSetup: validateLectureSetup(s.lectureSetup, ids, s.stages) }),
  };
}
