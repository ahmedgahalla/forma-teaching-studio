...
describe('render completion barrier', () => {
  ...
  it('unblocks canceled work promptly without resolving other scene waiters', async () => {
    ...
    const canceled = vi.fn(),
      drawn = vi.fn();
    const a = barrier.wait(first.signal).then(canceled),
      b = barrier.wait(second.signal).then(drawn);
    first.abort();
    await a;
  ...
  it('never subscribes an already-canceled request and removes listeners after completion', async () => {
    ...
    aborted.abort();
    const add = vi.spyOn(aborted.signal, 'addEventListener');
    await barrier.wait(aborted.signal);
    expect(add).not.toHaveBeenCalled();
    const current = new AbortController(),
      remove = vi.spyOn(current.signal, 'removeEventListener'),
      waiting = barrier.wait(current.signal);
    barrier.rendered();
    await waiting;
  ...
  it('keeps pending capture across a model renderer rebuild until the replacement draws', async () => {
    const barrier = createRenderBarrier(),
      ...
      waiting = barrier.wait(new AbortController().signal).then(done);
    barrier.reset();
    await Promise.resolve();
    expect(done).not.toHaveBeenCalled();
    barrier.rendered();
  ...
  it('rejects on a render failure and recovers only when a replacement renderer starts', async () => {
    const barrier = createRenderBarrier(),
      waiting = barrier.wait(new AbortController().signal),
      error = new Error('Graphics were interrupted.');
    const assertion = expect(waiting).rejects.toThrow('Graphics were interrupted.');
    barrier.fail(error);
    await assertion;
    barrier.rendered();
    await expect(barrier.wait(new AbortController().signal)).rejects.toThrow(
      'Graphics were interrupted.',
    );
    barrier.reset();
    const recovered = barrier.wait(new AbortController().signal);
    barrier.rendered();
  ...
  it('unblocks and cleans every remaining listener when the viewer unmounts', async () => {
    ...
    await waiting;
    expect(remove).toHaveBeenCalledOnce();
    await expect(barrier.wait(new AbortController().signal)).resolves.toBeUndefined();
    barrier.rendered();
    expect(remove).toHaveBeenCalledOnce();
  });
