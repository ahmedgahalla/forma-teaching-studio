    ...
    teeth: [
      {
        ...
        occlusal: [0, -1, 0],
        calibrated: true,
        geometry: mesh([-1, -2, 0, 1, -2, 0, -1, 2, 0]),
        rootGeometry: mesh([0, 3, 0, 1, 3, 0, 0, 15, 0]),
      },
    ],
...
function vertices(bytes: Uint8Array): number[][] {
  ...
  const result = Array.from({ length: position.count }, (_, index) => [
    position.getX(index),
    position.getY(index),
    position.getZ(index),
  ]);
...
function expectVertices(actual: number[][], expected: number[][]) {
  ...
  actual.forEach((point, i) =>
    point.forEach((n, axis) => expect(n).toBeCloseTo(expected[i][axis], 5)),
  );
}
...
afterEach(() => {
  ...
  meshes.splice(0).forEach(geometry => geometry.dispose());
});
...
describe('stage STL geometry', () => {
  it('bakes translation and rotation in the crown pose, preserves gums, and excludes roots', () => {
    const model = sampleCase(),
      before = triangleVertices(model.teeth[0].geometry);
  ...
  it('honors indexed triangle ordering and an absent identity pose', () => {
    const model = sampleCase();
    model.teeth[0].geometry.setIndex([2, 0, 1]);
    expectVertices(vertices(buildStageSTL(model, {}, false)).slice(0, 3), [
      [9, 22, 30],
      [9, 18, 30],
      [11, 18, 30],
    ]);
  ...
  it('inserts each configured attachment after its crown with exactly the same pose, without mutating or disposing source geometry', () => {
    ...
    const attachment = createAttachmentGeometry(model.teeth[0], spec);
    meshes.push(attachment);
    const local = triangleVertices(attachment),
      transforms = { '11': pose([2, -1, 3], [0, 0, 90]) };
    ...
    expectVertices(
      withAttachments.slice(6, 6 + local.length),
      local.map(([x, y, z]) => [12 - y, 19 + x, 33 + z]),
    );
  ...
  it.each<Transforms>([
    { '21': pose([0, 0, 0]) },
    { '11': pose([Infinity, 0, 0]) },
    { '11': pose([100001, 0, 0]) },
  ])('rejects invalid poses before writing %j', transforms => {
  ...
  it.each([-1, 0.5, 51, NaN, Infinity])('rejects invalid single-stage index %s', index => {
...
describe('geometric stage ZIP', () => {
  it('exports a saved nonzero baseline through checkpoints and freezes it before yielding', async () => {
    const model = sampleCase(),
      original = { '11': pose([3, 2, 0], [0, 0, 20]) },
      baseline = structuredClone(original);
    const checkpoints = [
        { id: 'a', name: 'Middle', transforms: { '11': pose([5, 4, 0], [0, 0, 40]) } },
      ],
      final = { '11': pose([9, 6, 0], [0, 0, 80]) };
    const pending = exportStageSequence(
      model,
      final,
      checkpoints,
      4,
    ...
    original['11'].translation[0] = 99;
    await pending;
    const archive = unzipSync(await downloadedBytes()),
      manifest = JSON.parse(strFromU8(archive['manifest.json']));
    expect(manifest.original).toEqual(baseline);
    expect(manifest.interpolation).toMatch(/saved original arrangement/);
    expectVertices(
      vertices(archive['stage-000.stl']),
      vertices(buildStageSTL(model, baseline, false)),
    );
    expectVertices(
      vertices(archive['stage-001.stl']),
      vertices(buildStageSTL(model, { '11': pose([4, 3, 0], [0, 0, 30]) }, false)),
    );
    expectVertices(
      vertices(archive['stage-002.stl']),
      vertices(buildStageSTL(model, checkpoints[0].transforms, false)),
    );
    expectVertices(
      vertices(archive['stage-004.stl']),
      vertices(buildStageSTL(model, final, false)),
    );
  });
  it('rejects an invalid baseline before encoding or downloading a sequence', async () => {
    ...
    await expect(
      exportStageSequence(sampleCase(), {}, [], 2, false, null, undefined, {
        '11': pose([Infinity, 0, 0]),
      }),
    ).rejects.toThrow(/transforms/);
  ...
  it('exports un-magnified initial response from its own fixed reference and freezes its assumptions', async () => {
    const model = sampleCase(),
      from = { '11': pose([2, 0, 0]) },
      to = { '11': pose([2.04, 0.02, 0], [0, 0, 2]) };
    const expectedFrom = structuredClone(from),
      expectedTo = structuredClone(to),
      assumptions = ['Virtual support; no remodeling.'];
    const pending = exportStageSequence(
      model,
      { '11': pose([50, 0, 0]) },
      [
        {
          id: 'unrelated',
          name: 'Unrelated geometric path',
          transforms: { '11': pose([20, 0, 0]) },
        },
      ],
      2,
      false,
    ...
    from['11'].translation[0] = 80;
    to['11'].translation[0] = 90;
    assumptions[0] = 'Changed later';
    await pending;
    const archive = unzipSync(await downloadedBytes()),
      manifest = JSON.parse(strFromU8(archive['manifest.json']));
    expect(manifest.initialResponse).toMatchObject({
    ...
    expect(manifest.trajectory).toBeUndefined();
    ...
    expectVertices(
      vertices(archive['stage-000.stl']),
      vertices(buildStageSTL(model, expectedFrom, false)),
    );
    expectVertices(
      vertices(archive['stage-001.stl']),
      vertices(buildStageSTL(model, { '11': pose([2.02, 0.01, 0], [0, 0, 1]) }, false)),
    );
    expectVertices(
      vertices(archive['stage-002.stl']),
      vertices(buildStageSTL(model, expectedTo, false)),
    );
  });

  it('rejects invalid or competing initial-response paths before encoding', async () => {
    const model = sampleCase(),
      preview = transitionTryMode(model, createTryState(), {
        type: 'preview',
        edit: { type: 'segment-translate', teeth: ['11'], axis: 'x', amount: 1 },
      }).pending!;
    const encode = vi.spyOn(STLExporter.prototype, 'parse');
    await expect(
      exportStageSequence(model, {}, [], 2, false, null, {
        from: {},
        to: { '11': pose([Infinity, 0, 0]) },
        assumptions: [],
      }),
    ).rejects.toThrow(/transforms/);
    await expect(
      exportStageSequence(model, {}, [], 2, false, null, { from: {}, to: {}, assumptions: [''] }),
    ).rejects.toThrow(/assumptions/);
    await expect(
      exportStageSequence(model, {}, [], 2, false, preview, { from: {}, to: {}, assumptions: [] }),
    ).rejects.toThrow(/one geometric path/);
    expect(encode).not.toHaveBeenCalled();
    expect(download).not.toHaveBeenCalled();
  });
  ...
  it('exports the displayed rigid segment arc, including a non-original start and exact intermediate manifest poses', async () => {
    ...
    const preview = transitionTryMode(model, createTryState(start), {
      type: 'preview',
      edit: { type: 'segment-rotate', teeth: ['11', '21'], axis: 'z', amount: 90 },
    }).pending!;
    const sourceGeometry = triangleVertices(model.teeth[0].geometry);
    const samples = [0, 0.5, 1].map(fraction => previewPose(preview, fraction));
    const exporting = exportStageSequence(
      model,
      preview.to,
      [{ id: 'unused', name: 'Unrelated checkpoint', transforms: { '11': pose([9, 0, 0]) } }],
      2,
      false,
      preview,
    );
    ...
    expect(manifest.stages.map((stage: { transforms: Transforms }) => stage.transforms)).toEqual(
      JSON.parse(JSON.stringify(samples)),
    );
    samples.forEach((sample, index) =>
      expectVertices(
        vertices(archive[`stage-00${index}.stl`]),
        vertices(buildStageSTL(model, sample, false)),
      ),
    );
    expectVertices(vertices(archive['stage-000.stl']).slice(0, 1), [[10, 20, 33]]);
    expectVertices(vertices(archive['stage-001.stl']).slice(0, 1), [
      [1 + 11 / Math.SQRT2, 22 + 7 / Math.SQRT2, 33],
    ]);
    ...
    const centres = model.teeth.map(tooth =>
      new THREE.Vector3(...tooth.position).add(
        new THREE.Vector3(...samples[1][tooth.id].translation),
      ),
    );
  ...
  it('exports stage 0, every interval and the final with checkpoint order and a clear manifest', async () => {
    ...
    expect(Object.keys(archive).sort()).toEqual([
      'manifest.json',
      ...Array.from({ length: 7 }, (_, i) => `stage-00${i}.stl`),
    ]);
  ...
  it('accepts the upper interval boundary and names all 51 stage files in order', async () => {
    ...
    expect(Object.keys(archive).filter(name => name.endsWith('.stl'))).toEqual(
      Array.from({ length: 51 }, (_, i) => `stage-${String(i).padStart(3, '0')}.stl`),
    );
  ...
  it('freezes the requested path across asynchronous yields', async () => {
    ...
    await pending;
    expectVertices(vertices(unzipSync(await downloadedBytes())['stage-002.stl']).slice(0, 1), [
      [11, 18, 30],
    ]);
  });
  ...
  it.each([1, 51, 2.5, NaN, Infinity])(
    'rejects invalid interval count %s before exporting',
    async count => {
      await expect(exportStageSequence(sampleCase(), {}, [], count, false)).rejects.toThrow(
        /Invalid stage/,
      );
      expect(download).not.toHaveBeenCalled();
    },
  ...
  it('rejects too many or invalid checkpoints without starting a download', async () => {
    ...
    await expect(
      exportStageSequence(
        model,
        {},
        Array.from({ length: 21 }, (_, i) => ({ id: String(i), name: 'Example', transforms: {} })),
        2,
        false,
      ),
    ).rejects.toThrow(/20 checkpoints/);
    await expect(
      exportStageSequence(
        model,
        {},
        [{ id: 'a', name: 'Example', transforms: { '21': pose([1, 0, 0]) } }],
        2,
        false,
      ),
    ).rejects.toThrow(/transforms/);
  ...
  it('guards raw batch size before allocating STL buffers while still allowing an individual stage', async () => {
    const model = sampleCase();
    model.teeth[0].geometry.setIndex(
      new THREE.BufferAttribute(
        new Uint32Array(330000).map((_, i) => i % 3),
        1,
      ),
    );
    const encode = vi.spyOn(STLExporter.prototype, 'parse');
    await expect(exportStageSequence(model, {}, [], 50, false)).rejects.toThrow(
      /250 MB.*individual stages/,
    );
    expect(encode).not.toHaveBeenCalled();
    expect(download).not.toHaveBeenCalled();
