...
export function calculateMechanics(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          'The calculation exceeded its time limit. Simplify the configuration and try again.',
        ),
      );
    }, 20000);
    ...
    worker.onerror = () => {
      finish();
      reject(new Error('The mechanics worker could not finish. Your setup is unchanged.'));
    };
    try {
      worker.postMessage({ requestId: experiment.revision, experiment });
    } catch (error) {
      finish();
      reject(error);
    }
