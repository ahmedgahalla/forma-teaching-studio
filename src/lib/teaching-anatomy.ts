...
export type AnatomyViewState = { bone: boolean; opacity: number; cutaway: boolean; ligament: boolean };
export type AnatomyLabel = { name: string; color: string; position: THREE.Vector3; side: 'left' | 'right' };
export const DEFAULT_ANATOMY: AnatomyViewState = { bone: false, opacity: .45, cutaway: false, ligament: false };
...
export function anatomyCutawayTooth(model: DentalCase, state: AnatomyViewState | undefined, selected: string) {
  return model.demo && state?.cutaway ? model.teeth.find(tooth => tooth.id === selected && tooth.rootGeometry && tooth.calibrated) : undefined;
}

function frameFor(tooth: DentalTooth): Frame {
  const axes = anatomicalFrame(tooth), buccal = new THREE.Vector3(...axes.buccal), rootward = new THREE.Vector3(...axes.occlusal).negate();
  return { rootward, buccal, right: new THREE.Vector3().crossVectors(rootward, buccal).normalize() };
}
...
function rootProfiles(tooth: DentalTooth, frame: Frame): Profile[][] {
  ...
  const branches = new Map<number, { points: THREE.Vector3[]; edges: [THREE.Vector3, THREE.Vector3][] }>(), point = new THREE.Vector3();
  const local: THREE.Vector3[] = [];
  for (let i = 0; i < positions.count; i++) {
    point.fromBufferAttribute(positions, i); const root = find(i), branch = branches.get(root) || { points: [], edges: [] };
    local[i] = new THREE.Vector3(point.dot(frame.right), point.dot(frame.rootward), point.dot(frame.buccal));
    branch.points.push(local[i]); branches.set(root, branch);
  }
  for (let i = 0; i < (index?.count || positions.count); i += 3) {
    const ids = [0, 1, 2].map(k => index ? index.getX(i + k) : i + k), branch = branches.get(find(ids[0]))!;
  ...
  return [...branches.values()].filter(branch => branch.points.length > 20).map(branch => {
    ...
    return Array.from({ length: 33 }, (_, i) => {
      ...
      for (const [a, b] of branch.edges) {
        if (Math.abs(a.y - b.y) < 1e-8 || sample < Math.min(a.y, b.y) || sample > Math.max(a.y, b.y)) continue;
        const t = (sample - a.y) / (b.y - a.y), x = THREE.MathUtils.lerp(a.x, b.x, t), z = THREE.MathUtils.lerp(a.z, b.z, t);
...
function sleeve(profiles: Profile[], frame: Frame, inner: number, outer: number, cutaway: boolean, bone: boolean) {
  ...
  const add = (point: THREE.Vector3, color: THREE.Color) => { const id = positions.length / 3; positions.push(...point.toArray()); colors.push(color.r, color.g, color.b); return id; };
  const paths = [inner, outer].map((offset, layer) => {
    const path = rings.map((p, j) => {
      const t = j / (rings.length - 1);
      return { ...p, axial: p.axial + (bone && layer ? .4 : 0),
        rx: bone && layer ? Math.max(p.rx + 1.35, rings[0].rx + outer - .65 * t) : p.rx + offset,
  ...
  const vertices = paths.map((path, layer) => path.map(p => {
    ...
    return Array.from({ length: cols }, (_, k) => {
      ...
      return add(fromFrame(frame, p.x + p.rx * Math.sin(angle), p.axial, p.z + p.rz * Math.cos(angle)), layer ? body : socket);
    });
  }));
  const count = paths[0].length, at = (layer: number, j: number, k: number) => vertices[layer][j][vertices[layer][j].length === 1 ? 0 : k % cols];
...
export function createTeachingAnatomy(model: DentalCase) {
  ...
  function cached(tooth: DentalTooth, cutaway: boolean) {
    const key = `${tooth.id}/${cutaway}`; let entry = cache.get(key); if (entry) return entry;
    const frame = frameFor(tooth), profiles = rootProfiles(tooth, frame);
    entry = { bone: profiles.map(p => sleeve(p, frame, .58, 1.8, cutaway, true)), ligament: profiles.map(p => sleeve(p, frame, .08, .46, cutaway, false)), profiles };
    cache.set(key, entry); return entry;
  }
  function update(transforms: Transforms, state: AnatomyViewState | undefined, options: Options) {
    if (disposed) return;
    group.clear(); group.visible = false; result.bounds.makeEmpty(); result.gumPlanes = []; result.labels = []; result.cutawayTooth = undefined;
    if (!model.demo || !state || (!state.bone && !state.ligament && !state.cutaway)) return;
    if (!Number.isFinite(state.opacity) || state.opacity < 0 || state.opacity > 1) throw new Error('Bone opacity must be between 0 and 1.');
    const cutaway = anatomyCutawayTooth(model, state, options.selected), opening = options.opening || 0;
    if (state.cutaway && !cutaway) return;
    result.cutawayTooth = cutaway;
    boneMaterial.opacity = state.opacity; boneMaterial.depthWrite = state.opacity >= 1;
    for (const tooth of model.teeth) {
      if (!tooth.rootGeometry || !tooth.calibrated || (cutaway ? tooth.id !== cutaway.id : options.arch && options.arch !== 'both' && toothArch(tooth.id) !== options.arch)) continue;
      const entry = cached(tooth, !!cutaway), offset = new THREE.Vector3(...tooth.position); if (toothArch(tooth.id) === 'lower') offset.y -= opening;
      for (const [name, shown, geometries, material] of [['bone', state.bone, entry.bone, boneMaterial], ['ligament', state.ligament, entry.ligament, ligamentMaterial]] as const) {
        if (!shown) continue;
        geometries.forEach((geometry, i) => { const mesh = new THREE.Mesh(geometry, material); mesh.name = `${name}-${tooth.id}-${i}`; mesh.position.copy(offset); group.add(mesh); result.bounds.union(geometry.boundingBox!.clone().translate(offset)); });
      }
    }
    if (cutaway) {
      const frame = frameFor(cutaway), root = cutaway.rootGeometry!, positions = root.getAttribute('position'), rootBox = new THREE.Box3().setFromBufferAttribute(positions as THREE.BufferAttribute);
      const rootCenter = rootBox.getCenter(new THREE.Vector3()), reference = new THREE.Vector3(...cutaway.position); if (toothArch(cutaway.id) === 'lower') reference.y -= opening;
      const posed = toothMatrix(cutaway, transforms); if (toothArch(cutaway.id) === 'lower') posed.elements[13] -= opening;
      const sourceBounds = rootBox.clone().union(new THREE.Box3().setFromBufferAttribute(cutaway.geometry.getAttribute('position') as THREE.BufferAttribute)).expandByScalar(2.3).translate(reference);
      const center = sourceBounds.getCenter(new THREE.Vector3()), half = sourceBounds.getSize(new THREE.Vector3()).multiplyScalar(.5);
      // World-space positive half-spaces keep only this socket's gingival fragment.
      for (const [axis, extent] of [[new THREE.Vector3(1, 0, 0), half.x], [new THREE.Vector3(0, 1, 0), half.y], [new THREE.Vector3(0, 0, 1), half.z]] as const) for (const sign of [-1, 1]) {
        const normal = axis.clone().multiplyScalar(sign); result.gumPlanes.push(new THREE.Plane().setFromNormalAndCoplanarPoint(normal, center.clone().addScaledVector(normal, -extent)));
      }
      ...
      if (options.gums !== false) {
        label('Gingiva', COLORS.gum, fromFrame(frame, neck.x - neck.rx - 1, neck.axial - .2, neck.z - .5).add(reference), 'right');
        for (const gum of model.gums.filter(gum => gum.arch === toothArch(cutaway.id))) { const offset = new THREE.Vector3(...gum.position); if (gum.arch === 'lower') offset.y -= opening; result.bounds.union(new THREE.Box3().setFromBufferAttribute(gum.geometry.getAttribute('position') as THREE.BufferAttribute).translate(offset).intersect(sourceBounds)); }
      }
      if (state.ligament) label('Periodontal ligament', COLORS.ligament, fromFrame(frame, middle.x + middle.rx + .27, middle.axial, middle.z).add(reference), 'right');
      if (state.bone) label('Supporting bone', COLORS.bone, fromFrame(frame, middle.x + middle.rx + 1.3, middle.axial, middle.z).add(reference), 'right');
      result.bounds.union(new THREE.Box3().setFromBufferAttribute(cutaway.geometry.getAttribute('position') as THREE.BufferAttribute).applyMatrix4(posed));
    }
...
export function layoutAnatomyLabels(labels: AnatomyLabel[], camera: THREE.Camera, width: number, height: number) {
  const compact = width < 520, labelWidth = Math.min(compact ? 121 : 170, Math.max(70, width * .3));
  const top = Math.min(96, height * .25), bottom = Math.max(top + 1, height - 48), row = Math.min(42, (bottom - top) / 3);
  const projected = labels.map(label => { const point = label.position.clone().project(camera); return { ...label, anchorX: (point.x + 1) * width / 2, anchorY: (1 - point.y) * height / 2, depth: point.z, x: label.side === 'left' ? 9 : width - labelWidth - 9, y: 0, width: labelWidth }; });
  for (const side of ['left', 'right']) {
    const column = projected.filter(label => label.side === side).sort((a, b) => a.anchorY - b.anchorY);
    column.forEach((label, i) => { label.y = THREE.MathUtils.clamp(label.anchorY - 14, top + i * row, bottom - (column.length - i) * row); });
    for (let i = 1; i < column.length; i++) column[i].y = Math.max(column[i].y, column[i - 1].y + row);
  }
  return projected;
}
