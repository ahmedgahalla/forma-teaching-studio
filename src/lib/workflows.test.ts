...
const teeth: Tooth[] = [1, 2, 3, 4].flatMap(quadrant =>
  Array.from({ length: 7 }, (_, index) => {
    const side = quadrant === 1 || quadrant === 4 ? -1 : 1;
    return {
      id: `${quadrant}${index + 1}`,
      name: 'Synthetic tooth',
      position: [side * (4 + index * 3), quadrant <= 2 ? 5 : -5, 18 - index * 4] as Vec3,
      buccal: [side * 0.6, 0, 0.8] as Vec3,
      mesial: [-side * 0.8, 0, 0.6] as Vec3,
      occlusal: [0, quadrant <= 2 ? -1 : 1, 0] as Vec3,
      calibrated: true,
...
describe('authored workflow storyboards', () => {
    ...
    for (const workflow of WORKFLOWS) {
      ...
      for (const step of workflow.steps) {
        for (const field of [
          ...
          expect(step[field].length).toBeGreaterThan(8);
  ...
  it('keeps palate imagery exclusive to the palatal workflow', () => {
    for (const workflow of WORKFLOWS)
      for (const step of workflow.steps)
        expect(step.palate).toBe(workflow.id === 'palatal-expansion');
  ...
  it.each([
    ...
    'routes the %s retention shortcut to its intended holding explanation',
    (id, index, title) => {
...
describe('deterministic synthetic workflow frames', () => {
  it.each(WORKFLOWS.map(w => w.id))(
    'keeps %s installation and force annotations stationary, then holds the completed endpoint',
    id => {
  ...
  it('aligns authored crowded incisors back to the original reference through an actual interpolated orientation', () => {
    ...
    expect(quaternion(middle.transforms['11'].rotation).angleTo(new Quaternion())).toBeCloseTo(
      quaternion(start.transforms['11'].rotation).angleTo(new Quaternion()) / 2,
      12,
    );
  ...
  it('uses local buccal displacement and outward crown inclination for archwire teaching, without a palate split', () => {
    ...
    for (const id of ['16', '26']) {
      ...
      expect(
        new Vector3(...start.transforms[id].translation).dot(new Vector3(...tooth.buccal)),
      ).toBeCloseTo(-0.7, 12);
      expect(
        new Vector3(...end.transforms[id].translation).dot(new Vector3(...tooth.buccal)),
      ).toBeCloseTo(0.55, 12);
      const originalLongAxis = new Vector3(...tooth.occlusal!);
      const crownDirection = originalLongAxis
        .clone()
        .applyQuaternion(quaternion(end.transforms[id].rotation));
      expect(
        crownDirection.sub(originalLongAxis).dot(new Vector3(...tooth.buccal)),
      ).toBeGreaterThan(0);
      expect(quaternion(end.transforms[id].rotation).angleTo(new Quaternion())).toBeCloseTo(
  ...
  it('is independent of visit order and never changes the source teeth or geometry', () => {
    ...
    expect(
      getWorkflowFrame('archwire-expansion', 4, 0.3, teeth).transforms['11'].translation[0],
    ).not.toBe(1000);
    expect(JSON.stringify(teeth)).toBe(before);
    for (const workflow of WORKFLOWS)
      for (const step of [0, 4, 6]) {
        const frame = getWorkflowFrame(workflow.id, step, 0.5, teeth);
        for (const t of teeth.filter(tooth => Number(tooth.id[0]) >= 3))
          expect(frame.transforms[t.id] ?? emptyPose()).toEqual(emptyPose());
      }
  });
  ...
  it.each([-0.1, 1.1, NaN, Infinity])(
    'rejects invalid progress %s without clamping it',
    progress => {
  ...
  it('rejects unknown workflows, missing teeth, duplicate IDs, corrupt coordinates and uncalibrated frames', () => {
    ...
    expect(() =>
      getWorkflowFrame(
        ...
        0,
        0,
        teeth.filter(t => t.id !== '11'),
      ),
    ).toThrow(/requires tooth 11/);
    expect(() =>
      getWorkflowFrame(
        'palatal-expansion',
        0,
        0,
        teeth.filter(t => t.id !== '17'),
      ),
    ).toThrow(/requires tooth 17/);
    ...
    expect(() =>
      getWorkflowFrame(
        ...
        0,
        0,
        teeth.map(t => (t.id === '11' ? { ...t, position: [NaN, 0, 0] } : t)),
      ),
    ).toThrow(/metadata/);
    expect(() =>
      getWorkflowFrame(
        ...
        0,
        0,
        teeth.map(t => (t.id === '16' ? { ...t, calibrated: false } : t)),
      ),
    ).toThrow(/calibrated/);
  });
});
