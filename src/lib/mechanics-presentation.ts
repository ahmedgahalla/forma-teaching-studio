...
export function recommendedMechanicsMagnification(size: ResponseSize): number {
  ...
  if (!Number.isFinite(distance) || !Number.isFinite(rotation) || distance < 0 || rotation < 0)
    throw new Error('Invalid calculated response size.');
...
export function mechanicsResponseCaption(size: ResponseSize, magnification: number): string {
  if (!hasMechanicsMovement(size))
    return 'No measurable response in this configuration. No movement is added for display.';
...
export function mechanicsDisplayPoses(
  ...
  if (
    ...
    progress < 0 ||
    progress > 1 ||
    !(MECHANICS_DISPLAY_SCALES as readonly number[]).includes(magnification)
  )
    throw new Error('Invalid mechanics presentation scale.');
  if (!progress) return reference;
  if (progress === 1 && magnification === 1) return result;
  const displayed: Transforms = {},
    factor = progress * magnification;
  for (const id of new Set([...Object.keys(reference), ...Object.keys(result)])) {
    const start = reference[id] || emptyPose(),
      end = result[id] || emptyPose();
    ...
    const q1 = new Quaternion().setFromEuler(
      new Euler(...(end.rotation.map(MathUtils.degToRad) as Vec3)),
    );
    const delta = q0.clone().invert().multiply(q1);
    if (delta.w < 0) delta.set(-delta.x, -delta.y, -delta.z, -delta.w);
    const halfAngle = Math.acos(MathUtils.clamp(delta.w, -1, 1)),
      sine = Math.sin(halfAngle);
    const scale = Math.abs(sine) < 1e-10 ? factor : Math.sin(halfAngle * factor) / sine;
    delta
      .set(delta.x * scale, delta.y * scale, delta.z * scale, Math.cos(halfAngle * factor))
      .normalize();
    const rotation = new Euler().setFromQuaternion(q0.multiply(delta), 'XYZ');
    displayed[id] = {
      translation: start.translation.map(
        (value, axis) => value + (end.translation[axis] - value) * factor,
      ) as Vec3,
...
export function explainMechanics(
  experiment: MechanicsExperiment,
  result: MechanicsResult | null = experiment.result,
): string {
  ...
  const parts = [
    `This is the initial elastic response of ${moving} teeth in a synthetic teaching model.`,
