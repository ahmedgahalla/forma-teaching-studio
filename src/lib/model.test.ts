...
describe('millimetre movement in the case frame', () => {
  ...
  it('makes opposing directions cancel', () => {
    for (const [forward, backward] of [
      ...
      expect(addMovement(moved, resolveMovement(tooth, backward, 1.5)).translation).toEqual([
        0, 0, 0,
      ]);
    }
  });
  ...
  it('rejects invalid amounts and invalid direction vectors', () => {
    for (const amount of [NaN, Infinity, -Infinity])
      expect(() => resolveMovement(tooth, 'x', amount)).toThrow();
...
describe('anatomical frames and orthodontic rotations', () => {
  ...
  const expectSameRotation = (a: Quaternion, b: Quaternion) =>
    expect(Math.abs(a.dot(b))).toBeCloseTo(1, 10);
  ...
  it('uses the declared local tip, torque and long axes and preserves translation', () => {
    for (const [movement, axis] of [
      ['tip', upper.buccal],
      ['torque', upper.mesial],
      ['rotate', upper.occlusal!],
    ] as const) {
      ...
      expectSameRotation(
        quaternion(rotated),
        new Quaternion().setFromAxisAngle(new Vector3(...axis), Math.PI / 6),
      );
  ...
  it('respects mirrored mesial frames and opposite upper/lower long axes', () => {
    ...
    expectSameRotation(
      quaternion(addOrthodonticRotation(emptyPose(), opposite, 'torque', 12)),
      quaternion(addOrthodonticRotation(emptyPose(), upper, 'torque', -12)),
    );
    expectSameRotation(
      quaternion(addOrthodonticRotation(emptyPose(), lower, 'rotate', 12)),
      quaternion(addOrthodonticRotation(emptyPose(), upper, 'rotate', -12)),
    );
  });
  ...
  it('composes in command order around fixed reference axes, including an exact inverse sequence', () => {
    ...
    const expected = new Quaternion()
      .setFromAxisAngle(new Vector3(...upper.occlusal!), MathUtils.degToRad(42))
      .multiply(
        new Quaternion().setFromAxisAngle(new Vector3(...upper.mesial), MathUtils.degToRad(-17)),
      )
      .multiply(
        new Quaternion().setFromAxisAngle(new Vector3(...upper.buccal), MathUtils.degToRad(25)),
      );
    expectSameRotation(quaternion(rotated), expected);
    const otherOrder = addOrthodonticRotation(
      addOrthodonticRotation(emptyPose(), upper, 'torque', -17),
      upper,
      'tip',
      25,
    );
    expect(Math.abs(quaternion(torqued).dot(quaternion(otherOrder)))).toBeLessThan(0.9999);
    const restored = addOrthodonticRotation(
      addOrthodonticRotation(
        addOrthodonticRotation(rotated, upper, 'rotate', -42),
        upper,
        'torque',
        17,
      ),
  ...
  it('also composes explicit world rotations about fixed axes after mixed movements', () => {
    ...
    const expected = new Quaternion()
      .setFromAxisAngle(new Vector3(1, 0, 0), MathUtils.degToRad(-22))
    ...
    expectSameRotation(
      quaternion(incremented),
      new Quaternion()
        .setFromAxisAngle(new Vector3(0, 1, 0), MathUtils.degToRad(10))
        .multiply(quaternion(saved)),
    );
  });
});
...
describe('atomic whole-selection operations', () => {
  ...
  it('moves every selected tooth the specified distance along its own reference direction', () => {
    ...
    const result = applyDentalCommand(original, teeth, {
      type: 'move_group',
      teeth: ['11', '21', '11'],
      direction: 'buccal',
      amount: 1,
    ...
    for (const pose of Object.values(result))
      expect(Math.hypot(...pose.translation)).toBeCloseTo(1, 12);
  ...
  it('does not mutate even the first tooth when a later selected tooth fails calibration', () => {
    ...
    for (const command of [
      ...
      expect(() =>
        applyDentalCommand(original, [teeth[0], { ...teeth[1], calibrated: false }], command),
      ).toThrow(/calibrated/);
  ...
  it('rejects missing members, empty groups, invalid poses and out-of-range direct calls', () => {
    expect(() => applyDentalCommand({}, teeth, { type: 'reset', teeth: ['11', '41'] })).toThrow(
      /not present/,
    );
    expect(() => applyDentalCommand({}, teeth, { type: 'reset', teeth: [] })).toThrow(/at least/);
    expect(() =>
      applyDentalCommand({ '21': { translation: [0, NaN, 0], rotation: [0, 0, 0] } }, teeth, {
        type: 'move_group',
        teeth: ['11', '21'],
        direction: 'x',
        amount: 1,
    ...
    for (const amount of [0, 11, -11, Infinity, NaN])
      expect(() =>
        applyDentalCommand({}, teeth, {
          type: 'move_group',
          teeth: ['11', '21'],
          direction: 'x',
          amount,
    ...
    for (const amount of [0, 181, -181, Infinity, NaN])
      expect(() =>
        applyDentalCommand({}, teeth, {
          type: 'orthodontic',
          teeth: ['11', '21'],
  ...
  it('resets only requested teeth and handles group world rotation without requiring calibration', () => {
    const initial: Transforms = {
      '11': addMovement(emptyPose(), [1, 2, 3]),
      '21': addMovement(emptyPose(), [3, 2, 1]),
    };
    ...
    const rotated = applyDentalCommand(
      initial,
      teeth.map(t => ({ ...t, calibrated: false })),
      { type: 'rotate_group', teeth: ['11', '21'], axis: 'x', amount: 5 },
    );
...
describe('immutable poses for history', () => {
  ...
  it('returns independent zero poses and rejects arithmetic overflow', () => {
    ...
    expect(() =>
      addMovement({ translation: [Number.MAX_VALUE, 0, 0], rotation: [0, 0, 0] }, [
        Number.MAX_VALUE,
        0,
        0,
      ]),
  ...
  it('validates complete saved poses and rejects corrupted input before an operation', () => {
    ...
    for (const value of [
      ...
      { translation: [0, Infinity, 0], rotation: [0, 0, 0] },
    ]) {
