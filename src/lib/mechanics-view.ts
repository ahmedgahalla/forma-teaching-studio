...
export function createMechanicsVisuals(model: DentalCase) {
  ...
  const geometries: THREE.BufferGeometry[] = [],
    transientMaterials: THREE.Material[] = [];
  const clear = () => {
    group.clear();
    geometries.splice(0).forEach(geometry => geometry.dispose());
    transientMaterials.splice(0).forEach(material => material.dispose());
  };
  const mesh = (
  ...
  const tube = (
    ...
    mesh(
      new THREE.TubeGeometry(curve, Math.max(12, points.length * 8), radius, 8, false),
      material,
    );
  };
  const update = (
    experiment: MechanicsExperiment | null | undefined,
    poses: Transforms,
    display: Display,
  ) => {
    clear();
    ...
    const endpoint = (end: MechanicsEndpoint) =>
      end.kind === 'tooth' ? world(end.tooth, end.local) : tadPosition(end.id);
    const endpointVisible = (end: MechanicsEndpoint) =>
      end.kind === 'tooth'
        ? display.visible(end.tooth)
        : display.visible(nearest(config.tads.find(item => item.id === end.id)!.position).id);
    for (const wire of config.wires) {
      const points = wire.teeth.filter(display.visible).map(anchor);
      ...
      if (wire.section.shape === 'round') tube(points, wire.section.diameterMm / 2, material, true);
        ...
        const halfW = wire.section.widthMm / 2,
          halfH = wire.section.heightMm / 2,
        ...
        mesh(
          new THREE.ExtrudeGeometry(shape, {
            steps: Math.max(12, points.length * 8),
            bevelEnabled: false,
            extrudePath: new THREE.CatmullRomCurve3(points, false, 'centripetal'),
          }),
          material,
        );
      }
    ...
    for (const elastic of config.elastics)
      if (endpointVisible(elastic.from) && endpointVisible(elastic.to)) {
        const from = endpoint(elastic.from),
          to = endpoint(elastic.to);
        tube([from, to], 0.13, elasticMaterial);
        mesh(new THREE.SphereGeometry(0.25, 10, 6), elasticMaterial, from);
        mesh(new THREE.SphereGeometry(0.25, 10, 6), elasticMaterial, to);
      }
    for (const expander of config.expanders) {
      ...
      const left = mean(expander.left),
        right = mean(expander.right),
      ...
      const screw = mesh(new THREE.CylinderGeometry(0.4, 0.4, 5, 12), metal, center);
      screw.rotation.z = Math.PI / 2;
      for (const id of [...expander.left, ...expander.right])
        if (display.visible(id))
          tube(
            [
              anchor(id),
              center.clone().add(new THREE.Vector3(id.startsWith('2') ? 2.4 : -2.4, 0, 0)),
            ],
            0.38,
            metal,
          );
    ...
    if (display.forces && display.revealed && experiment.result) {
      const maximum = Math.max(
        0.001,
        ...experiment.result.teeth.map(tooth => Math.hypot(...tooth.forceN)),
      );
      for (const tooth of experiment.result.teeth) {
        ...
        if (moment.length() > 1e-6) {
          ...
          const geometry = new THREE.BufferGeometry().setFromPoints(points),
            material = new THREE.LineBasicMaterial({ color: momentColor });
          geometries.push(geometry);
