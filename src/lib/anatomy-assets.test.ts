...
function fixture() {
  ...
  const gums = ['upper', 'lower'].map(arch => {
    const mesh = new Mesh(new BoxGeometry(70, 10, 35), material);
    mesh.name = `gum_${arch}`;
    scene.add(mesh);
  ...
  return {
    scene,
    metadata: { version: 1, name: 'Bundled synthetic model', units: 'mm', teeth, gums },
  };
}
...
describe('Blender anatomy asset contract', () => {
  it('loads the shipped Blender GLB with all linked teeth and source millimetre pivots', async () => {
    ...
    const result = dentalCaseFromAsset(
      scene.scene,
    ...
    expect(result.gums).toHaveLength(2);
    for (const tooth of result.teeth) {
      ...
      expect(
        tooth.geometry.boundingBox!.min.distanceTo(original.geometry.boundingBox!.min),
      ).toBeLessThan(0.5);
      expect(
        tooth.geometry.boundingBox!.max.distanceTo(original.geometry.boundingBox!.max),
      ).toBeLessThan(0.5);
      // The joined trunk and curved fuller root body intentionally change silhouette.
      expect(
        tooth.rootGeometry!.boundingBox!.min.distanceTo(original.rootGeometry!.boundingBox!.min),
      ).toBeLessThan(1.5);
    }
    const triangles = result.teeth.reduce(
      (count, tooth) =>
        count +
        (tooth.geometry.index?.count || tooth.geometry.getAttribute('position').count) / 3 +
        (tooth.rootGeometry!.index?.count || tooth.rootGeometry!.getAttribute('position').count) /
          3,
      0,
    );
  ...
  it('preserves supplied pivots and linked roots without recentering', () => {
    const { scene, metadata } = fixture(),
      result = dentalCaseFromAsset(scene, metadata);
  ...
  it('loads connected root trunks with explicit distal branches and registered cutaways', async () => {
    ...
    const model = dentalCaseFromAsset(scene.scene, raw),
      anatomy = createTeachingAnatomy(model);
    for (const tooth of model.teeth) {
      const root = tooth.rootGeometry!,
        index = root.index!,
        p = root.getAttribute('position');
      ...
      for (let i = 0; i < index.count; i += 3) {
        parent[find(index.getX(i + 1))] = find(index.getX(i));
        parent[find(index.getX(i + 2))] = find(index.getX(i));
      }
      expect(
        new Set(parent.map((_, i) => find(i))).size,
        `${tooth.id} connected root surface`,
      ).toBe(1);
      const upper = Number(tooth.id[0]) < 3,
        family = Number(tooth.id[1]),
        count = family >= 6 ? (upper ? 3 : 2) : upper && family === 4 ? 2 : 1;
      expect(tooth.rootAnatomy!.branches).toHaveLength(count);
      expect(!!tooth.rootAnatomy!.trunk).toBe(count > 1);
      const saved = [...p.array];
      anatomy.update(
        {},
        { bone: true, opacity: 0.5, ligament: true, cutaway: true },
        { selected: tooth.id, roots: true, gums: true },
      );
      ...
      for (const child of anatomy.group.children as Mesh[]) {
        ...
        expect(Array.from(child.geometry.getAttribute('normal').array).every(Number.isFinite)).toBe(
          true,
        );
      }
      expect([...p.array]).toEqual(saved);
      expect(tooth.rootAnatomy).not.toBe(
        raw.teeth.find((item: { id: string }) => item.id === tooth.id).rootAnatomy,
      );
    }
    anatomy.dispose();
  });
  it('retains distinguishing posterior landmarks in the exported surfaces, not just the Blender source', async () => {
    ...
    const model = dentalCaseFromAsset(
      scene.scene,
      JSON.parse(readFileSync('public/models/forma-teaching-v1.json', 'utf8')),
    );
    ...
    const height = (id: string, u: number, v: number) => {
      ...
      const axis = new Vector3(...tooth.occlusal!),
        mesial = new Vector3(...tooth.mesial),
        buccal = new Vector3(...tooth.buccal),
        point = new Vector3(),
        positions = tooth.geometry.getAttribute('position');
      let minM = Infinity,
        maxM = -Infinity,
        minB = Infinity,
        maxB = -Infinity,
      ...
      for (let i = 0; i < positions.count; i++) {
        ...
        minM = Math.min(minM, point.dot(mesial));
        maxM = Math.max(maxM, point.dot(mesial));
        minB = Math.min(minB, point.dot(buccal));
        maxB = Math.max(maxB, point.dot(buccal));
        maxA = Math.max(maxA, point.dot(axis));
      }
      const origin = mesial
        .multiplyScalar((minM + maxM) / 2 + ((maxM - minM) * u) / 2)
        .addScaledVector(buccal, (minB + maxB) / 2 + ((maxB - minB) * v) / 2)
        .addScaledVector(axis, maxA + 5);
      const hit = new Raycaster(origin, axis.clone().negate()).intersectObject(
        new Mesh(tooth.geometry, material),
      )[0];
      expect(hit, `${id} occlusal landmark ray`).toBeDefined();
      return hit.point.dot(axis) - maxA;
    };
    // Lower first premolar: dominant buccal cusp; second: two separated lingual lobes.
    expect(height('44', 0, 0.45) - height('44', 0, -0.45)).toBeGreaterThan(0.65);
    for (const u of [-0.3, 0.3])
      expect(height('45', u, -0.4) - height('45', 0, -0.45)).toBeGreaterThan(0.2);
    // Upper first premolar has a more unequal cusp pair than the second.
    expect(height('14', 0, 0.45) - height('14', 0, -0.45)).toBeGreaterThan(
      height('15', 0, 0.45) - height('15', 0, -0.45) + 0.15,
    );
    ...
    expect(height('16', -0.3, -0.4) - height('17', -0.3, -0.4)).toBeGreaterThan(0.15);
    expect(height('46', -0.7, 0) - height('47', -0.7, 0)).toBeGreaterThan(0.2);
    material.dispose();
  });
  it('applies parent transforms exactly once', () => {
    ...
    expect(
      (scene.getObjectByName('crown_11') as Mesh).geometry.getAttribute('position').getZ(0),
    ).toBe(2.5);
  });
  it('rejects missing and duplicate FDI inventory, malformed axes and wrong units', () => {
    ...
    expect(() =>
      validateAnatomyMetadata({ ...metadata, teeth: metadata.teeth.slice(1) }),
    ).toThrow();
    expect(() =>
      validateAnatomyMetadata({
        ...metadata,
        teeth: metadata.teeth.map((t, i) => (i === 1 ? { ...t, id: '11' } : t)),
      }),
    ).toThrow(/unique/);
    expect(() =>
      validateAnatomyMetadata({
        ...metadata,
        teeth: metadata.teeth.map((t, i) => (i === 0 ? { ...t, buccal: [1, 0, 0] } : t)),
      }),
    ).toThrow(/perpendicular/);
  });
  it('rejects missing meshes, invalid positions and implausible units', () => {
    const { scene, metadata } = fixture();
    const crown = scene.getObjectByName('crown_11') as Mesh;
    crown.name = 'unidentified';
    expect(() => dentalCaseFromAsset(scene, metadata)).toThrow(/Missing/);
    crown.name = 'crown_11';
    crown.geometry.getAttribute('position').setX(0, NaN);
