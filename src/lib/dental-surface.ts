...
export function dentalCavity(source: THREE.BufferGeometry): Float32Array {
  ...
  const point = new THREE.Vector3(),
    ...
    raw = new Float32Array(positions.count);
  for (let i = 0; i < positions.count; i++) {
    ...
    for (const j of neighbours[i]) {
      delta.fromBufferAttribute(positions, j).sub(point);
      const distance = delta.length();
      if (distance < 1e-8) continue;
      sum.addScaledVector(delta, 1 / distance);
      weight += 1 / distance;
      length += distance;
    }
    if (weight)
      raw[i] = THREE.MathUtils.clamp(
        (sum.divideScalar(weight).dot(normal) / (length / neighbours[i].size) - 0.015) * 3,
        0,
        1,
      );
  }
  ...
  for (let i = 0; i < result.length; i++) {
    let sum = raw[i] * 2;
    for (const j of neighbours[i]) sum += raw[j];
...
export function dentalSurface(
  ...
  let low = Infinity,
    high = -Infinity;
  for (let i = 0; i < positions.count; i++) {
    const height = point.fromBufferAttribute(positions, i).dot(axis);
  ...
  for (let i = 0; i < positions.count; i++) {
    const t =
      (point.fromBufferAttribute(positions, i).dot(axis) - low) / Math.max(high - low, 0.001);
    const bodyMix = THREE.MathUtils.smoothstep(t, 0, tissue === 'gingiva' ? 0.65 : 0.46);
    color
      .copy(cervical)
      .lerp(body, bodyMix)
...
export function updateDentalBackdrop(texture: THREE.DataTexture, theme: DentalStageTheme) {
  const size = texture.image.width,
    ...
    palette = dentalStagePalette[theme];
  const edge = new THREE.Color(palette.edge),
    center = new THREE.Color(palette.center),
    color = new THREE.Color();
  for (let y = 0; y < size; y++)
    for (let x = 0; x < size; x++) {
      const radius = Math.hypot((x / (size - 1) - 0.5) * 1.15, (y / (size - 1) - 0.55) * 1.4);
      color
        .copy(center)
        .lerp(edge, THREE.MathUtils.smoothstep(radius, 0.05, 0.85))
        .convertLinearToSRGB();
...
export function dentalBackdrop(theme: DentalStageTheme = 'midnight') {
  const size = 96,
    texture = new THREE.DataTexture(new Uint8Array(size * size * 4), size, size);
