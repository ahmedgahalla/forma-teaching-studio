...
export function toothName(id: string) { return names[Number(id[1]) - 1] || 'Third molar'; }

export function createDemo(): DentalCase { return getTeachingAssetCase() || createOrthodonticDemo(); }
...
export async function importSTLs(files: File[], scale: number): Promise<DentalCase> {
  ...
  if (files.reduce((n, f) => n + f.size, 0) > 100 * 1024 * 1024) throw new Error('Keep the total STL upload below 100 MB.');
  const teeth: DentalTooth[] = [], gums: Gum[] = [], ids = new Set<string>();
  let totalVertices = 0;
  try {
    for (const file of files) {
      ...
      if (!id && !isGum) throw new Error(`${file.name}: name each tooth by its FDI number, for example 11.stl, or name a gum file upper_gum.stl.`);
      ...
      if (data.byteLength >= 84) { const count = new DataView(data).getUint32(80, true); if (84 + count * 50 === data.byteLength && count > 1_000_000) throw new Error(`${file.name}: mesh exceeds 1 million triangles.`); }
      const geometry = new STLLoader().parse(data);
      const positions = geometry.getAttribute('position');
      totalVertices += positions?.count || 0;
      if (!positions || positions.count < 3 || positions.count % 3 || totalVertices > 6_000_000 || !Array.from(positions.array).every(Number.isFinite)) { geometry.dispose(); throw new Error(`${file.name}: invalid or overly large triangle mesh.`); }
      ...
      if (!Number.isFinite(extent) || !center.toArray().every(Number.isFinite) || extent <= .001 || extent > 2000) { geometry.dispose(); throw new Error(`${file.name}: check the selected units and mesh dimensions.`); }
      geometry.translate(-center.x, -center.y, -center.z); geometry.computeVertexNormals();
      const position = center.toArray() as Vec3;
      if (isGum) gums.push({ id: file.name, geometry, position, arch: /lower|mandib/i.test(file.name) ? 'lower' : /upper|maxill/i.test(file.name) ? 'upper' : undefined });
      else teeth.push({ id: id!, name: toothName(id!), geometry, position, buccal: [0, 0, 1], mesial: [1, 0, 0], calibrated: false });
    }
    if (!teeth.length) throw new Error('Include at least one numbered tooth STL.');
    return { name: 'Imported arch', demo: false, teeth: teeth.sort((a, b) => a.id.localeCompare(b.id)), gums };
  } catch (error) { teeth.forEach(t => t.geometry.dispose()); gums.forEach(g => g.geometry.dispose()); throw error; }
}
...
function serialGeometry(g: THREE.BufferGeometry): SerializedGeometry {
  return { vertices: Array.from(g.getAttribute('position').array), normals: Array.from(g.getAttribute('normal')?.array || []), indices: g.index ? Array.from(g.index.array) : undefined };
}
...
const finiteVec = (v: unknown): v is Vec3 => Array.isArray(v) && v.length === 3 && v.every(x => typeof x === 'number' && Number.isFinite(x) && Math.abs(x) <= 1e5);
function validMesh(value: SerializedGeometry | undefined): boolean {
  if (!value || !Array.isArray(value.vertices) || value.vertices.length < 9 || value.vertices.length % (value.indices === undefined ? 9 : 3) || !value.vertices.every(v => typeof v === 'number' && Number.isFinite(v) && Math.abs(v) < 1e5)) return false;
  if (value.indices !== undefined && (!Array.isArray(value.indices) || value.indices.length < 3 || value.indices.length % 3 || !value.indices.every(i => Number.isInteger(i) && i >= 0 && i < value.vertices.length / 3))) return false;
  return value.normals === undefined || (Array.isArray(value.normals) && (value.normals.length === 0 || value.normals.length === value.vertices.length) && value.normals.every(v => typeof v === 'number' && Number.isFinite(v) && Math.abs(v) <= 2));
}
export async function loadCase(file: File): Promise<{ model: DentalCase; transforms: Transforms; session?: CaseSession }> {
  ...
  for (const t of [...data.model.teeth, ...data.model.gums]) {
    if (typeof t.id !== 'string' || !finiteVec(t.position) || !validMesh(t)) throw new Error('Invalid mesh in case.');
    ...
    count += t.vertices.length + (t.indices?.length || 0) + (t.root?.vertices.length || 0) + (t.root?.indices?.length || 0); if (count > 18_000_000) throw new Error('Case mesh is too large.');
  }
  for (const t of data.model.teeth) {
    if (!/^[1-4][1-8]$/.test(t.id) || ids.has(t.id) || typeof t.name !== 'string' || !finiteVec(t.buccal) || !finiteVec(t.mesial) || typeof t.calibrated !== 'boolean') throw new Error('Invalid tooth metadata.');
    if ((t.occlusal !== undefined && !finiteVec(t.occlusal)) || (t.bracketPosition !== undefined && !finiteVec(t.bracketPosition))) throw new Error('Invalid tooth metadata.');
  ...
  for (const [id, pose] of Object.entries(data.transforms) as [string, {translation: unknown; rotation: unknown}][]) if (!ids.has(id) || !pose || !finiteVec(pose.translation) || !finiteVec(pose.rotation)) throw new Error('Invalid movement data.');
  const session = validateSession(data.session, ids);
  const makeGeometry = (value: SerializedGeometry) => {
    const g = new THREE.BufferGeometry(); g.setAttribute('position', new THREE.Float32BufferAttribute(value.vertices, 3));
    if (value.indices) g.setIndex(value.indices);
    if (value.normals?.length) g.setAttribute('normal', new THREE.Float32BufferAttribute(value.normals, 3)); else g.computeVertexNormals();
    return g;
  };
...
export function exportSTL(model: DentalCase, transforms: Transforms) {
  ...
  for (const tooth of model.teeth) { const mesh = new THREE.Mesh(tooth.geometry); const pose = transforms[tooth.id]; mesh.position.fromArray(tooth.position); if (pose) { mesh.position.add(new THREE.Vector3(...pose.translation)); mesh.rotation.set(...pose.rotation.map(v => THREE.MathUtils.degToRad(v)) as Vec3); } group.add(mesh); }
  for (const gum of model.gums) { const mesh = new THREE.Mesh(gum.geometry); mesh.position.fromArray(gum.position); group.add(mesh); }
