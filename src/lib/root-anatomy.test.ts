...
const source: RootAnatomy = {
  version: 1,
  trunk: [
    { center: [0, 0, 0], radii: [3, 3] },
    { center: [0, 2, 0], radii: [2, 2] },
  ],
  branches: [
    [
      { center: [1, 2, 0], radii: [1, 1] },
      { center: [1.5, 10, 0], radii: [0.025, 0.025] },
    ],
    [
      { center: [-1, 2, 0], radii: [1, 1] },
      { center: [-1.5, 10, 0], radii: [0.025, 0.025] },
...
describe('explicit synthetic root sections', () => {
  ...
  it('accepts a single root without a trunk and rejects reversed apical order', () => {
    expect(
      validateRootAnatomy({ version: 1, branches: [source.branches[0]] }, [0, -1, 0]).trunk,
    ).toBeUndefined();
  ...
  it.each([
    ...
    { ...source, version: 2 },
    { ...source, branches: [] },
    { ...source, branches: Array(4).fill(source.branches[0]) },
    { ...source, trunk: [] },
    { ...source, branches: [[{ center: [0, 0, 0], radii: [1, 1] }]] },
  ])('rejects malformed or unbounded inventory %#', raw => {
  ...
  it.each([NaN, Infinity, -1, 0, 13])('rejects invalid section radius %s', value => {
    ...
    raw.branches[0][0].radii[0] = value;
    expect(() => validateRootAnatomy(raw)).toThrow(/root anatomy/);
  });
  it('rejects nonfinite coordinates, implausible paths and duplicate sections', () => {
    for (const center of [
      ...
      const raw = structuredClone(source);
      raw.branches[0][1].center = center as [number, number, number];
      expect(() => validateRootAnatomy(raw)).toThrow(/root anatomy/);
    }
  });
});
