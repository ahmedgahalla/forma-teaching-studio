...
export function sameViewerGeometry(previous: DentalCase, next: DentalCase): boolean {
  if (previous.teeth.length !== next.teeth.length || previous.gums.length !== next.gums.length) return false;
  ...
  for (const tooth of next.teeth) {
    ...
    if (!before || before.geometry !== tooth.geometry || before.rootGeometry !== tooth.rootGeometry || !samePosition(before.position, tooth.position)) return false;
  }
  const gums = new Map(previous.gums.map(gum => [gum.id, gum]));
  return next.gums.every(gum => {
    const before = gums.get(gum.id);
    return !!before && before.geometry === gum.geometry && samePosition(before.position, gum.position);
  });
}
