...
describe('perspective fit for every dental view', () => {
  ...
  const views = [
    ['perspective', new Vector3(.38, .2, 1.3)],
    ['front', new Vector3(0, 0, 1)],
    ['right', new Vector3(-1, .03, 0)],
    ['upper occlusal', new Vector3(0, -1, .001)],
    ['lower occlusal', new Vector3(0, 1, .001)],
  ] as const;

  it.each(views)('keeps all corners inside the %s camera with margin at desktop and portrait aspects', (_, direction) => {
    for (const aspect of [850 / 525, 2.5, 0.55]) {
      const camera = new PerspectiveCamera(34, aspect, .1, 10000);
      const center = bounds.getCenter(new Vector3());
      camera.position.copy(center).addScaledVector(direction.clone().normalize(), perspectiveFitDistance(bounds, direction, camera.up, camera.fov, aspect));
  ...
  it('accounts for near-corner depth, rather than only fitting the central plane', () => {
    ...
    const distance = perspectiveFitDistance(deep, new Vector3(0, 0, 1), new Vector3(0, 1, 0), 90, 1, 1);
    expect(distance).toBeCloseTo(60, 12);
  });

  it('is invariant to world offset and direction magnitude, and leaves inputs unchanged', () => {
    const direction = new Vector3(.38, .2, 1.3), up = new Vector3(0, 1, 0);
    ...
    expect(perspectiveFitDistance(shifted, direction.clone().multiplyScalar(9), up, 34, 1.6)).toBeCloseTo(distance, 10);
  ...
  it('requires more distance when a horizontal field of view shrinks', () => {
    ...
    const landscape = perspectiveFitDistance(wide, new Vector3(0, 0, 1), new Vector3(0, 1, 0), 34, 2);
    const portrait = perspectiveFitDistance(wide, new Vector3(0, 0, 1), new Vector3(0, 1, 0), 34, 0.5);
