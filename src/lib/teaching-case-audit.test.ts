...
type CaseAudit = {
  baseline: { crownPairs: Pair[]; rootPairs: Pair[] };
  variants: Record<string, { sampleCount: number; frames: Frame[] }>;
};
  ...
  it('covers every variant, every authored keyframe and all advertised intermediate samples', () => {
    ...
    for (const definition of TEACHING_CASES) {
      ...
      for (const variant of definition.variants) {
        ...
        expect(result.frames.map(item => item.progress)).toEqual(
          [...new Set([...audit.samples, ...variant.keyframes.map(item => item.progress)])].sort(
            (a, b) => a - b,
          ),
        );
      ...
      for (const variant of Object.values(report.variants))
        for (const frame of variant.frames) {
          for (const component of ['crownPairs', 'rootPairs'] as const) {
            const pairs = frame[component],
              before = new Set(report.baseline[component].map(key));
            expect(new Set(pairs.map(key)).size).toBe(pairs.length);
            for (const pair of pairs) {
              expect(pair.a).toMatch(/^[1-4][1-7]$/);
              expect(pair.b).toMatch(/^[1-4][1-7]$/);
              expect(Number(pair.a)).toBeLessThan(Number(pair.b));
              if (definition.id === 'anchorage-space-closure') {
                expect([pair.a, pair.b]).not.toContain('14');
                expect([pair.a, pair.b]).not.toContain('24');
              }
            }
            expect(frame[component === 'crownPairs' ? 'newCrownPairs' : 'newRootPairs']).toEqual(
              pairs.filter(pair => !before.has(key(pair))),
            );
          }
        }
    }
