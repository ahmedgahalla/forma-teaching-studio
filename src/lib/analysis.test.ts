...
const tooth = (id: string, position: Vec3, size: Vec3 = [2, 2, 2]): DentalTooth => ({ id, name: id, position, geometry: new BoxGeometry(...size), buccal: [0, 0, 1], mesial: [1, 0, 0], occlusal: [0, -1, 0], calibrated: true });
const model = (...teeth: DentalTooth[]): DentalCase => ({ name: 'Test case', demo: false, teeth, gums: [] });
...
describe('surface intersection at transformed tooth poses', () => {
  it('ignores separated meshes and detects true triangle-surface intersections', () => {
    ...
    expect(findSurfaceIntersections(dental, { '21': { translation: [-1, 0.2, 0.1], rotation: [0, 0, 0] } })).toEqual([{ a: '11', b: '21' }]);
    expect(findSurfaceIntersections(dental, {})).toEqual([]);
  });
  ...
  it('uses narrow-phase surfaces to reject overlapping axis-aligned bounding boxes', () => {
    ...
    const dental = model(tooth('11', [0, 0, 0], [4, 0.2, 0.2]), tooth('21', [-0.5, 0.5, 0], [4, 0.2, 0.2]));
    const transforms: Transforms = { '11': { translation: [0, 0, 0], rotation: [0, 0, 45] }, '21': { translation: [0, 0, 0], rotation: [0, 0, 45] } };
  ...
  it('accounts for rotation, original position and relative transforms of both objects', () => {
    const dental = model(tooth('11', [10, 3, 2], [4, 0.3, 0.3]), tooth('21', [10, 4.3, 2], [0.8, 0.8, 0.8]));
    expect(findSurfaceIntersections(dental, {})).toEqual([]);
    ...
    const together: Transforms = { '11': { translation: [50, -30, 11], rotation: [0, 0, 90] }, '21': { translation: [50, -30, 11], rotation: [0, 0, 0] } };
    expect(findSurfaceIntersections(dental, together)).toEqual([{ a: '11', b: '21' }]);
    expect(findSurfaceIntersections(dental, { ...together, '21': { translation: [50, -30, 15], rotation: [0, 0, 0] } })).toEqual([]);
  });

  it('does not mislabel complete containment as a triangle-surface intersection', () => {
    expect(findSurfaceIntersections(model(tooth('11', [0, 0, 0], [4, 4, 4]), tooth('21', [0, 0, 0], [0.5, 0.5, 0.5])), {})).toEqual([]);
  });
  ...
  it('does not change input poses, tooth positions, mesh vertices or mesh indices', () => {
    ...
    const transforms: Transforms = { '11': { translation: [0.1, 0.2, 0.3], rotation: [10, 20, 30] } };
    const before = JSON.stringify({ transforms, positions: dental.teeth.map(t => t.position), vertices: dental.teeth.map(t => Array.from(t.geometry.getAttribute('position').array)), indices: dental.teeth.map(t => Array.from(t.geometry.index!.array)) });
    findSurfaceIntersections(dental, transforms);
    findSurfaceIntersections(dental, transforms);
    const after = JSON.stringify({ transforms, positions: dental.teeth.map(t => t.position), vertices: dental.teeth.map(t => Array.from(t.geometry.getAttribute('position').array)), indices: dental.teeth.map(t => Array.from(t.geometry.index!.array)) });
  ...
  it('handles empty/single-tooth cases and never emits duplicate pairs', () => {
    ...
    const overlaps = findSurfaceIntersections(model(tooth('11', [0, 0, 0]), tooth('12', [0.4, 0.3, 0.2]), tooth('21', [0.8, 0.6, 0.4])), {});
    expect(overlaps).toEqual([{ a: '11', b: '12' }, { a: '11', b: '21' }, { a: '12', b: '21' }]);
  });
...
describe('millimetre centre measurements and movement summaries', () => {
  ...
  it('measures world-space model centres with translations and ignores local rotation', () => {
    ...
    expect(centreDistance(dental, { '13': { translation: [3, 0, 0], rotation: [65, 32, 11] } }, '13', '23')).toBe(4);
  ...
  it('reports only arch pairs present in the case with initial and final distances', () => {
    const dental = model(tooth('13', [-12, 0, 0]), tooth('23', [12, 0, 0]), tooth('16', [-20, 0, 0]), tooth('26', [20, 0, 0]), tooth('43', [-10, 4, 0]));
    const spans = archSpans(dental, { '13': { translation: [-1, 0, 0], rotation: [0, 0, 0] }, '23': { translation: [1, 0, 0], rotation: [0, 0, 0] } });
  ...
  it('reports displacement magnitude and geometric orientation change rather than sum of Euler angles', () => {
    ...
    const rows = movementRows(dental, { '11': { translation: [3, 4, 12], rotation: [0, 0, 270] }, '21': emptyPose() });
    ...
    const mixed = movementRows(dental, { '11': { translation: [0, 0, 0], rotation: [90, 90, 0] } })[0];
