...
export function toothMatrix(tooth: DentalTooth, transforms: Transforms): Matrix4 {
  ...
  return new Matrix4().compose(
    new Vector3(...tooth.position).add(new Vector3(...pose.translation)),
    new Quaternion().setFromEuler(new Euler(...(pose.rotation.map(MathUtils.degToRad) as Vec3))),
    new Vector3(1, 1, 1),
  );
}
...
function intersectsUsingBothTrees(tree: MeshBVH, geometry: BufferGeometry, relative: Matrix4) {
  ...
  if (prior && !prior.configurable && (!('writable' in prior) || !prior.writable))
    return tree.intersectsGeometry(geometry, relative);
  const other = surfaceTree(geometry);
  Object.defineProperty(
    geometry,
    'boundsTree',
    prior && !prior.configurable
      ? { value: other }
      : { value: other, writable: true, configurable: true },
  );
  try {
    return tree.intersectsGeometry(geometry, relative);
  } finally {
    if (prior) Object.defineProperty(geometry, 'boundsTree', prior);
...
export function findSurfaceIntersections(
  ...
  const pieces = model.teeth.map(tooth => {
    if (!tooth.geometry.boundingBox) tooth.geometry.computeBoundingBox();
    const matrix = toothMatrix(tooth, transforms);
    return { tooth, matrix, box: tooth.geometry.boundingBox!.clone().applyMatrix4(matrix) };
  });
  ...
  for (let i = 0; i < pieces.length; i++)
    for (let j = i + 1; j < pieces.length; j++) {
      ...
      if (intersectsUsingBothTrees(tree, b.tooth.geometry, relative))
...
export function centreDistance(model: DentalCase, transforms: Transforms, a: string, b: string) {
  const first = model.teeth.find(t => t.id === a),
    second = model.teeth.find(t => t.id === b);
  if (!first || !second) return null;
  const p = new Vector3(...first.position).add(
    new Vector3(...(transforms[a]?.translation || [0, 0, 0])),
  );
  const q = new Vector3(...second.position).add(
    new Vector3(...(transforms[b]?.translation || [0, 0, 0])),
  );
  return p.distanceTo(q);
}
export function archSpans(model: DentalCase, transforms: Transforms, original: Transforms = {}) {
  return [
    { name: 'Upper canine centres', a: '13', b: '23' },
    { name: 'Lower canine centres', a: '43', b: '33' },
    { name: 'Upper first-molar centres', a: '16', b: '26' },
    { name: 'Lower first-molar centres', a: '46', b: '36' },
  ]
    .map(pair => ({
      ...pair,
      initial: centreDistance(model, original, pair.a, pair.b),
      final: centreDistance(model, transforms, pair.a, pair.b),
    }))
    .filter(pair => pair.initial !== null && pair.final !== null);
}
export function movementRows(model: DentalCase, transforms: Transforms, original: Transforms = {}) {
  return model.teeth.map(tooth => {
    const pose = transforms[tooth.id] || emptyPose(),
      start = original[tooth.id] || emptyPose();
    const delta = pose.translation.map((value, axis) => value - start.translation[axis]);
    const q = new Quaternion().setFromEuler(
      new Euler(...(pose.rotation.map(MathUtils.degToRad) as Vec3)),
    );
    const initial = new Quaternion().setFromEuler(
      new Euler(...(start.rotation.map(MathUtils.degToRad) as Vec3)),
    );
    return {
      id: tooth.id,
      name: tooth.name,
      ...
      orientationChange: MathUtils.radToDeg(q.angleTo(initial)),
    };
  });
}
