...
import { historyReducer, interpolateTransforms, stageTransforms, validateSession, validTransforms, type CaseSession, type Plan } from './planning';

const pose = (translation: Vec3, rotation: Vec3 = [0, 0, 0]): Pose => ({ translation, rotation });
const quaternion = (p: Pose) => new Quaternion().setFromEuler(new Euler(...p.rotation.map(MathUtils.degToRad) as Vec3));

describe('whole-operation history', () => {
  const teeth: Tooth[] = ['11', '21'].map(id => ({ id, name: id, position: [0, 0, 0], buccal: [0, 0, 1], mesial: [1, 0, 0], occlusal: [0, -1, 0], calibrated: true }));
  ...
  it('undoes and redoes a multi-tooth command as one snapshot with its label intact', () => {
    ...
    const moved = applyDentalCommand(original.current, teeth, { type: 'move_group', teeth: ['11', '21'], direction: 'buccal', amount: 0.5 });
    const committed = historyReducer(original, { type: 'commit', value: moved, label: 'Move both incisors' });
  ...
  it('keeps redo order across several undos and discards only the old branch on a new commit', () => {
    ...
    const a = historyReducer(start, { type: 'commit', value: { '11': pose([1, 0, 0]) }, label: 'A' });
    ...
    const branch = historyReducer(twiceUndone, { type: 'commit', value: { '21': pose([0, 1, 0]) }, label: 'New branch' });
  ...
  it('loads saved histories and handles empty undo/redo without inventing entries', () => {
    ...
    const loaded = historyReducer(start, { type: 'load', value: { '11': pose([1, 0, 0]) }, past, future });
...
describe('checkpoint paths and quaternion stage previews', () => {
  ...
  it('takes the shortest orientation path across the Euler wrap boundary', () => {
    const result = interpolateTransforms({ '11': pose([0, 0, 0], [0, 0, 170]) }, { '11': pose([0, 0, 0], [0, 0, -170]) }, 0.5);
  ...
  it('interpolates combined rotations as orientations rather than linear Euler components', () => {
    const a = pose([0, 0, 0], [90, 0, 0]), b = pose([2, 4, 6], [0, 90, 0]);
  ...
  it.each([[0, 1], [0, 2.5], [-1, 10], [11, 10], [NaN, 10], [Infinity, 10]])('rejects invalid stage %s and count %s', (stage, count) => {
...
describe('restored session validation', () => {
  ...
  const valid = (): CaseSession => ({ stages: 10, checkpoints: [{ id: 'first', name: 'First', transforms: { '11': emptyPose() } }], past: [{ value: {}, label: 'First move' }], future: [], braces: true, roots: false, bracketStyle: 'metal', ligatureColor: '#57a6e3' });
  ...
  it('accepts complete sessions, legacy cases without a session, and empty transform maps', () => {
    ...
    expect(validateSession({ ...valid(), bracketStyle: 'ceramic', ligatureColor: '#AABBCC' }, ids)?.bracketStyle).toBe('ceramic');
  ...
  it.each([true, false])('preserves attachment visibility %s through a serialized session', attachments => {
  ...
  it.each([null, 0, 1, 'true', 'false', [], {}])('rejects non-boolean saved attachment visibility %j', attachments => {
  ...
  it.each(['none', 'brackets', 'braces', 'expander-bands', 'palatal-expander', 'retainer'] as const)('restores the %s display without changing poses or appliance visibility', preset => {
    const original = { ...valid(), braces: false, applianceDisplay: { preset, progress: 0.375, palate: true } };
    const encoded = JSON.stringify(original), decoded = JSON.parse(encoded);
  ...
  it.each([NaN, Infinity, -Infinity])('rejects nonfinite appliance progress %s before serialization', progress => {
    expect(() => validateSession({ ...valid(), applianceDisplay: { preset: 'braces', progress, palate: false } }, ids)).toThrow(/progress/);
  });

  it.each([null, [], { '31': emptyPose() }, { '11': { translation: [0, 0], rotation: [0, 0, 0] } }, { '11': pose([0, NaN, 0]) }, { '11': pose([0, Infinity, 0]) }, { '11': pose([100001, 0, 0]) }])('rejects malformed or unrecognized transforms: %j', value => {
    expect(validTransforms(value, ids)).toBe(false);
  });

  it.each([
    ['stages', 1], ['stages', 51], ['stages', 3.5], ['braces', 'true'], ['roots', null],
    ['bracketStyle', 'plastic'], ['ligatureColor', '#fff'], ['ligatureColor', 'red'],
    ['past', [{ value: { '31': emptyPose() }, label: 'Bad tooth' }]],
    ['future', [{ value: {}, label: 'x'.repeat(300) }]],
    ['past', Array(5001).fill({ value: {}, label: 'Move' })],
    ['checkpoints', Array.from({ length: 21 }, (_, i) => ({ id: String(i), name: 'Stage', transforms: {} }))],
    ['checkpoints', [{ id: 'a', name: 'x'.repeat(61), transforms: {} }]],
    ['checkpoints', [{ id: 'a', name: 'A', transforms: { '31': emptyPose() } }]],
    ['checkpoints', [{ id: 'a', name: 'A', transforms: {} }, { id: 'a', name: 'B', transforms: {} }]],
