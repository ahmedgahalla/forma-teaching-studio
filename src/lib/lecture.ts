...
function spokenInteger(words: string): number {
  ...
  return hundred === -1
    ? pieces.reduce((sum, word) => sum + wordValues[word], 0)
    : wordValues[pieces[0]] * 100 +
        pieces.slice(hundred + 1).reduce((sum, word) => sum + wordValues[word], 0);
}
...
export function normalizeSpeechCommand(text: string): string {
  let normalized = normalizeClassroomLanguage(text)
    .replace(/\s+/g, ' ')
    .replace(/^please /, '')
    .replace(/ please[.!]?$/, '')
    .replace(/[.!?]$/, '');
  ...
  normalized = normalized.replace(
    new RegExp(`\\b(${wholeNumber}) and (?:a )?half\\b`, 'g'),
    (_, words: string) => String(spokenInteger(words) + 0.5),
  );
  normalized = normalized.replace(
    /\b(?:a half|half(?: a)?) (?=millimet(?:er|re)s?\b|mm\b)/g,
    '0.5 ',
  );
  normalized = normalized.replace(
    /\b(?:a quarter|quarter(?: a)?) (?=millimet(?:er|re)s?\b|mm\b)/g,
    '0.25 ',
  );
  normalized = normalized.replace(
    new RegExp(`\\bpoint (${digit}(?: ${digit})*)\\b`, 'g'),
    (_, words: string) =>
      `point ${words
        .split(' ')
        .map(word => wordValues[word])
        .join('')}`,
  );
  normalized = normalized.replace(new RegExp(`\\b${wholeNumber}\\b`, 'g'), words =>
    String(spokenInteger(words)),
  );
  normalized = normalized
    .replace(/\b(\d+) point (\d+)\b/g, '$1.$2')
    .replace(/\bpoint (\d+)\b/g, '0.$1');
  normalized = normalized
    .replace(/\b(minus|negative) (?=\d)/g, '-')
    .replace(/\b(?:plus|positive) (?=\d)/g, '+');
  normalized = normalized
...
export function parseTeachingCommand(
  ...
  if (/^(?:try this setup|explore this setup|explore this lesson)$/.test(command))
    return { kind: 'workspace', action: 'explore' };
  if (/^(?:restore my workspace|back to my saved case)$/.test(command))
    return { kind: 'workspace', action: 'restore' };
  if (command === 'return to source lesson') return { kind: 'workspace', action: 'lesson' };
  const appliancePresets: Record<
    string,
    Extract<TeachingAction, { kind: 'appliance-display' }>['preset']
  > = {
    'place brackets only': 'brackets',
    'place braces': 'braces',
    'place expander bands': 'expander-bands',
    'place palatal expander': 'palatal-expander',
    'place fixed retainer': 'retainer',
    'remove teaching appliance': 'none',
  };
  if (Object.hasOwn(appliancePresets, command))
    return { kind: 'appliance-display', preset: appliancePresets[command] };
  const history = command.match(
    /^(undo|redo) (?:the )?(?:last )?(\d+)(?: (?:changes?|requests?|commands?))?$/,
  );
  if (history) {
    const count = Number(history[2]);
    if (!Number.isInteger(count) || count < 1 || count > 10)
      throw new CommandValidationError('Undo or redo between 1 and 10 complete requests.');
    return { kind: 'history', action: history[1] as 'undo' | 'redo', count };
  }
  if (/^(?:undo|redo)(?: that| the last (?:command|request))?$/.test(command))
    return { kind: 'dental', command: { type: command.startsWith('undo') ? 'undo' : 'redo' } };
  if (
    /^(?:repeat(?: that| it)?|replay(?: that| it)?|do that again)(?: (?:more )?slowly| slower)?$/.test(
      command,
    )
  )
    return { kind: 'replay', slower: /slowly|slower/.test(command) };
  if (/^(?:return|go back|back) to (?:the )?lesson$/.test(command))
    return { kind: 'return-lesson' };
  if (/^(?:explain|narrate|read) (?:this|the|current) step$/.test(command))
    return { kind: 'narrate', target: 'step' };
  if (/^(?:explain|narrate|read) (?:the )?answer(?: aloud)?$/.test(command))
    return { kind: 'narrate', target: 'answer' };
  ...
  if (/^(?:start|show|open) (?:the )?(?:tooth )?anatomy lesson$/.test(command))
    return { kind: 'anatomy-lesson', action: 'start' };
  if (command === 'compare translation and tipping')
    return { kind: 'anatomy-lesson', action: 'start' };
  const anatomyLesson = command.match(
    /^(?:demonstrate|show) (?:tooth )?(translation|tipping)(?: (?:in the )?anatomy lesson)?$/,
  );
  if (anatomyLesson)
    return { kind: 'anatomy-lesson', action: anatomyLesson[1] as 'translation' | 'tipping' };
  const anatomy = command.match(
    /^(show|hide) (?:the )?(bone|alveolar bone|cutaway|cutaway view|ligament|periodontal ligament|pdl)$/,
  );
  if (anatomy)
    return {
      kind: 'anatomy',
      action: /bone/.test(anatomy[2])
        ? 'bone'
        : /cutaway/.test(anatomy[2])
          ? 'cutaway'
          : 'ligament',
      visible: anatomy[1] === 'show',
    };
  ...
  if (boneAppearance)
    return {
      kind: 'anatomy',
      action: 'opacity',
      value: boneAppearance[1] === 'transparent' ? 0.25 : 1,
    };
  const opacity = command.match(/^(?:set )?(?:bone )?opacity (?:to )?([\d.]+)(?:\s*(%|percent))?$/);
  if (opacity) {
    const value = Number(opacity[1]) / (opacity[2] ? 100 : 1);
    if (!Number.isFinite(value) || value < 0 || value > 1)
      throw new CommandValidationError('Use bone opacity from 0 to 1, or 0 to 100 percent.');
    return { kind: 'anatomy', action: 'opacity', value };
  }
  const speed = command.match(
    /^(?:(?:set )?(?:playback )?speed (?:to )?)?(slow|half speed|normal speed|normal|fast|double speed|0\.5x|1x|2x)$/,
  );
  if (speed)
    return {
      kind: 'speed',
      value: ['slow', 'half speed', '0.5x'].includes(speed[1])
        ? 0.5
        : ['fast', 'double speed', '2x'].includes(speed[1])
          ? 2
          : 1,
    };
  const workflow = command.match(
    /^start (braces|fixed braces|palatal expansion|archwire expansion) workflow$/,
  );
  if (workflow)
    return {
      kind: 'workflow',
      action: 'start',
      id:
        workflow[1] === 'palatal expansion'
          ? 'palatal-expansion'
          : workflow[1] === 'archwire expansion'
            ? 'archwire-expansion'
            : 'fixed-braces',
    };
  ...
  const workflowPhases: Record<string, WorkflowPhase> = {
    'install brackets': 'brackets',
    'bond brackets': 'brackets',
    'insert archwire': 'wire',
    'engage archwire': 'wire',
    'install expander': 'wire',
    'fit expander': 'wire',
    'show forces': 'forces',
    'activate expander': 'forces',
    'show movement': 'movement',
    'demonstrate movement': 'movement',
    'show retention': 'retention',
  };
  if (Object.hasOwn(workflowPhases, command))
    return { kind: 'workflow', action: 'phase', phase: workflowPhases[command] };
  ...
  const view = command.match(
    /^(?:(?:show|switch to) (?:the )?)?(front|right|left|occlusal|3 ?d|perspective)(?: view)?$/,
  );
  if (view)
    return {
      kind: 'view',
      view:
        view[1] === '3d' || view[1] === '3 d'
          ? 'perspective'
          : (view[1] as 'front' | 'right' | 'left' | 'occlusal' | 'perspective'),
    };
  const arch = command.match(
    /^(?:show|isolate) (?:the )?(upper|lower|both|all) (?:arch|arches|teeth|jaws?)$/,
  );
  if (arch)
    return {
      kind: 'arch',
      arch: arch[1] === 'all' || arch[1] === 'both' ? 'both' : (arch[1] as 'upper' | 'lower'),
    };

  if (/^(?:show )?(?:before|initial)(?: positions?)?$/.test(command))
    return { kind: 'comparison', mode: 'before' };
  if (/^(?:show )?(?:after|final)(?: positions?)?$/.test(command))
    return { kind: 'comparison', mode: 'after' };
  if (
    /^(?:show (?:the )?original(?: overlay| positions?)?|show (?:comparison|overlay)|comparison on|overlay on)$/.test(
      command,
    )
  )
    return { kind: 'comparison', mode: 'overlay' };
  if (
    /^(?:hide (?:the )?original(?: overlay| positions?)?|hide (?:comparison|overlay)|comparison off|overlay off)$/.test(
      command,
    )
  )
    return { kind: 'comparison', mode: 'off' };

  const toggle = command.match(/^(show|hide) (?:the )?(.+)$/);
  if (toggle && Object.hasOwn(toggleNames, toggle[2]))
    return { kind: 'toggle', target: toggleNames[toggle[2]], visible: toggle[1] === 'show' };
  const toggleSuffix = command.match(/^(?:turn (?:the )?)?(.+) (on|off)$/);
  if (toggleSuffix && Object.hasOwn(toggleNames, toggleSuffix[1]))
    return {
      kind: 'toggle',
      target: toggleNames[toggleSuffix[1]],
      visible: toggleSuffix[2] === 'on',
    };
  ...
  if (exactStage) {
    ...
    if (!Number.isInteger(value) || value < 0 || value > 50)
      throw new CommandValidationError('Choose a display stage from 0 to 50.');
    return { kind: 'stage', action: 'exact', stage: value };
  }
  if (/^(?:stop|pause)(?: animation| playback)?$/.test(command)) return { kind: 'stop' };
  if (/^(?:lecture mode on|enter lecture mode|start lecture)$/.test(command))
    return { kind: 'lecture', enabled: true };
  if (/^(?:lecture mode off|exit lecture mode|end lecture)$/.test(command))
    return { kind: 'lecture', enabled: false };
  ...
  const attachment = command.match(
    /^(add|remove) (?:(rectangular|rectangle|ellipsoid|ellipsoidal|beveled|bevelled) )?attachments? (to|on|from) (.+)$/,
  );
  if (attachment) {
    const teeth = select(attachment[4]);
    if (attachment[1] === 'remove') {
      if (attachment[2] || attachment[3] !== 'from')
        throw new CommandValidationError('Use “remove attachments from” and name the teeth.');
      return { kind: 'attachment', action: 'remove', teeth };
    }
    if (attachment[3] === 'from')
      throw new CommandValidationError('Use “add attachments to” and name the teeth.');
    const shape = attachment[2]?.startsWith('ellipsoid')
      ? 'ellipsoid'
      : attachment[2]?.startsWith('bevel')
        ? 'beveled'
        : 'rectangle';
...
export type TeachingLesson = {
  ...
  title: string;
  description: string;
  steps: { command: string; caption: string }[];
};
